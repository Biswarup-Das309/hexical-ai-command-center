...
export async function GET(request: Request, context: { params: Promise<{ executionId: string }> }): Promise<Response> {
  ...
  try {
    ...
    if (!UUID_PATTERN.test(rawExecutionId))
      return jsonFailure(400, 'INVALID_EXECUTION_ID', 'The execution identifier is invalid.')
    ...
    if (requestedSessionIdRaw !== null && !UUID_PATTERN.test(requestedSessionIdRaw))
      return jsonFailure(400, 'INVALID_SESSION_ID', 'The session identifier is invalid.')
    ...
    log.error('tty.stream.open_failed', {
      correlationId,

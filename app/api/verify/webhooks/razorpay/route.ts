...
export async function POST(req: Request) {
  try {
    ...
    const { NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, RAZORPAY_WEBHOOK_SECRET } = process.env
    ...
    if (typeof amountPaid !== 'number' || !Number.isFinite(amountPaid) || amountPaid < expectedAmount) {
      console.error(
        `[WEBHOOK_FRAUD]: User ${clerkUserId} paid ${String(
          amountPaid,
        )} for ${targetTier} (expected >= ${expectedAmount})`,
      )

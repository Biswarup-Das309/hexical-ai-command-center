...
import { ASTDiffResult, ASTDiffNode } from '@/lib/hexical-types'
...
const DiffNodeCard = React.memo(({ node, type, measure, expandSignal, collapseSignal }: DiffNodeCardProps) => {
  ...
  const renderPayload = useCallback(
    (payload: string, label: string, colorClass: string, copyType: 'before' | 'after') => {
      const isOversized = payload.length > MAX_PAYLOAD_CHARS
      const displayStr =
        !isOversized || showFullPayload
          ? payload
      ...
      return (
        <div className="flex flex-col h-full min-w-0">
          ...
          <pre
            className={`p-3 rounded-md bg-black/50 ${colorClass} overflow-x-auto custom-scrollbar text-xs flex-1 border border-white/5`}
          >
            {displayStr}
          </pre>
        </div>
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between p-4 border-b border-zinc-800 bg-zinc-900/50 gap-4 shrink-0">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 text-xs font-mono text-zinc-400 w-full xl:w-auto">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-black/40 hover:bg-white/10 hover:text-zinc-200 px-2.5 py-1 rounded border border-zinc-800 transition-colors mr-2"
          >
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-4 p-3 border-b border-zinc-800 bg-zinc-900/20 shrink-0">
          ...
          <div className="flex items-center gap-2 w-full lg:w-auto justify-end">
            <button
              ...
              className="flex items-center gap-1 text-xs font-mono text-zinc-400 hover:text-zinc-200 px-2 py-1 rounded bg-black/30 border border-zinc-800 transition-colors"
            >

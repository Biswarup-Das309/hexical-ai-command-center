...
const markdownComponents = {
  h1: ({ node: _node, ...props }: MarkdownComponentProps<'h1'>) => (
    <h1
      className="text-2xl font-bold text-foreground mt-6 mb-4 tracking-tight"
      {...props}
    />
  ),
  h2: ({ node: _node, ...props }: MarkdownComponentProps<'h2'>) => (
    <h2
      className="text-xl font-semibold text-foreground mt-5 mb-3 border-b border-border/50 pb-2"
      {...props}
    />
  ),
  ...
  ul: ({ node: _node, ...props }: MarkdownComponentProps<'ul'>) => (
    <ul
      className="list-disc list-outside ml-6 mb-4 space-y-2 marker:text-cyan"
      {...props}
    />
  ),
  ol: ({ node: _node, ...props }: MarkdownComponentProps<'ol'>) => (
    <ol
      className="list-decimal list-outside ml-6 mb-4 space-y-2 marker:text-cyan font-mono"
      {...props}
    />
  ),
  ...
  strong: ({ node: _node, ...props }: MarkdownComponentProps<'strong'>) => (
    <strong
      className="font-bold text-cyan drop-shadow-[0_0_8px_rgba(0,255,255,0.3)]"
      {...props}
    />
  ),
  blockquote: ({ node: _node, ...props }: MarkdownComponentProps<'blockquote'>) => (
    <blockquote
      className="border-l-2 border-cyan pl-4 italic text-muted-foreground my-4 bg-muted/10 py-2 rounded-r-lg"
      {...props}
    />
  ),
  code: ({
    ...
    return match ? (
      <pre className="bg-card border border-border rounded-lg p-4 my-4 overflow-x-auto shadow-lg">
        <code
          className={`font-mono text-[13px] text-cyan ${className || ''}`}
          {...props}
        >
          {children}
        </code>
      ...
      <code
        className="bg-muted/50 text-cyan px-1.5 py-0.5 rounded font-mono text-[13px] border border-cyan/20"
        {...props}
      >
        {children}
      </code>

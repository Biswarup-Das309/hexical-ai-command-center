...
export function HexicalLogo({ className = 'size-8' }: { className?: string }) {
  return (
    ...
    >
      <defs>
        {/* The Hexical Signature Gradient: Vibrant Cyan to Deep Indigo */}
        ...
        {/* Glow Filter for the core element */}
        <filter id="glow">
      ...
      <path
        ...
        fill="url(#hexical-gradient)"
        className="opacity-80 hover:opacity-100 transition-all duration-500 delay-100"
      />
      <path
        d="M10 33 L22 40 L22 60 L10 67 Z"
        fill="url(#hexical-gradient)"
        className="opacity-70 hover:opacity-100 transition-all duration-500 delay-200"
      />
      <path
      ...
      {/* Central Core Element with Glow */}
      <circle cx="50" cy="50" r="8" fill="#22d3ee" filter="url(#glow)" className="animate-pulse" />
    </svg>
  )
}

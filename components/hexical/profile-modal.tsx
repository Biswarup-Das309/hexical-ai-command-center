  ...
  return (
    <div className="absolute bottom-20 left-4 w-[320px] bg-[#202124] border border-white/10 rounded-3xl shadow-2xl z-[1000] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
      ...
      <div className="flex justify-between items-center p-3">
        ...
        <button onClick={onClose} className="p-1 hover:bg-white/10 rounded-full text-foreground"><X size={18} /></button>
      </div>
        ...
        <>
          <div className="flex flex-col items-center pt-2 pb-6">
            <div className="relative mb-4">
              {user.avatar ? (
                <img src={user.avatar} alt={user.name} className="size-20 rounded-full border-2 border-[#202124] ring-2 ring-cyan/50 object-cover" />
              ) : (
          ...
          <div className="p-2 border-t border-white/5">
            <button onClick={onSignOut} className="w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-white/5 rounded-xl transition-all text-foreground">
        ...
        <div className="px-6 pb-8 pt-2">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            <div className="relative">
              ...
              <input type="email" placeholder="Email" required className="w-full bg-[#1a1b1e] p-2 pl-10 rounded-lg border border-white/10 focus:border-cyan-500 outline-none" value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div className="relative">
              <Lock className="absolute left-3 top-3 text-muted-foreground" size={16} />
              <input type="password" placeholder="Password" required className="w-full bg-[#1a1b1e] p-2 pl-10 rounded-lg border border-white/10 focus:border-cyan-500 outline-none" value={password} onChange={(e) => setPassword(e.target.value)} />
            </div>
            <button disabled={isLoading} className="w-full bg-cyan-600 py-2 rounded-lg font-medium hover:bg-cyan-500 transition-colors">
              {isLoading ? 'Processing...' : (authMode === 'login' ? 'Sign In' : 'Sign Up')}
            </button>
          </form>

          <button onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')} className="text-xs text-cyan mt-4 w-full text-center underline">

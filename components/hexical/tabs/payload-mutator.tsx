...
const ENCODING_TYPES: EncodingType[] = [
  'base64',
  'url',
  'hex',
  'unicode',
...
function encodeValue(source: string | Uint8Array, type: EncodingType): string {
  ...
  switch (type) {
    ...
    case 'ascii85':
      return ascii85EncodeBytes(bytes)
    case 'binary':
      return binaryEncodeBytes(bytes)
    case 'url':
...
function decodeValue(input: string, type: EncodingType): string {
  switch (type) {
    case 'base64':
      return bytesToText(base64DecodeBytes(input))
    case 'hex':
      return bytesToText(hexDecodeBytes(input))
    case 'base32':
      return bytesToText(base32DecodeBytes(input))
    case 'ascii85':
      return bytesToText(ascii85DecodeBytes(input))
    case 'binary':
      return bytesToText(binaryDecodeBytes(input))
    case 'url':
...
function TextTool({
  ...
  return (
    <div className="flex flex-col gap-6 h-full">
      <div
        className={`flex flex-col flex-1 gap-2 rounded-xl transition-all ${
          dragOver ? `${t.bg} ring-2 ${t.border}` : ''
        }`}
        onDragOver={(e) => {
          e.preventDefault()
          setDragOver(true)
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center bg-white/5 rounded-lg p-1 text-xs font-bold uppercase tracking-wider">
            <button
              ...
              className={`px-3 py-1.5 rounded-md transition-all ${
                mode === 'encode' ? `${t.bg} ${t.text}` : 'text-zinc-500 hover:text-white'
              }`}
            >
              Encode
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-md transition-all ${
                mode === 'decode' ? `${t.bg} ${t.text}` : 'text-zinc-500 hover:text-white'
              }`}
            >
              Decode
            </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-lg text-zinc-500 hover:text-white hover:bg-white/5 disabled:opacity-30 disabled:hover:bg-transparent transition-all"
            >
        ...
        {BYTE_TYPES.has(encType) && (
          <p className="text-[11px] text-zinc-600">
            Byte-safe: works on raw bytes, so dropped files encode correctly even when they aren't text.
          </p>
        )}
      </div>
      ...
      <div className="flex flex-col flex-1 gap-2">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs text-zinc-400 hover:text-white flex items-center gap-1 disabled:opacity-30"
          >
            {copied ? <Check size={12} /> : <Copy size={12} />} {copied ? 'Copied' : 'Copy'}
          </button>
        </div>
        <div
          className={`flex-1 bg-black border rounded-xl p-4 font-mono text-sm overflow-y-auto break-all shadow-inner ${
            error ? 'border-rose-500/30 text-rose-400' : `${t.border} ${t.text}`
          }`}
        >
  ...
  return (
    <div className="flex flex-col gap-6 h-full">
      ...
      <p className="text-xs text-zinc-600 leading-relaxed">
        MD5 and SHA-1 are broken for collision resistance — fine for checksums or cache keys, not for passwords,
        signatures, or anything security-sensitive. Prefer SHA-256+ generally, and a dedicated password hash (bcrypt,
        scrypt, or Argon2) for credentials specifically.
      </p>
    </div>
  )
}
...
function JwtTool({ theme }: { theme: AccentTheme }) {
  ...
  const buildToken = useCallback(async () => {
    ...
    try {
      ...
      if (!encodeSecret) {
        setBuildError('Enter a secret to sign the token')
        return
      }
    ...
    <div className="flex flex-col gap-6 h-full">
      ...
      {mode === 'decode' ? (
        <div className="flex flex-col gap-4 flex-1 min-h-0">
          ...
          {decodeError ? (
            <div className="border border-rose-500/30 text-rose-400 rounded-xl p-4 text-sm font-mono">
              ⚠ {decodeError}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-2 min-h-0">
                ...
                <pre className={`bg-black border rounded-xl p-3 font-mono text-xs overflow-auto ${t.border} ${t.text}`}>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 rounded-xl border border-white/10 text-xs text-zinc-400 hover:text-white"
              >
            ...
            {verifyState === 'unsupported' && (
              <span className="text-xs text-amber-400">
                Verification only supports HS256 (this token uses {header?.alg ?? 'unknown'}).
              </span>
            )}
            {verifyState === 'error' && (
              <span className="text-xs text-rose-400">Could not verify — check the secret.</span>
            )}
          </div>
          <p className="text-xs text-zinc-600 leading-relaxed">
            Decoding just reads the header and payload — anyone can do that without the secret. A JWT is signed, not
            encrypted, so treat its contents as visible to anyone who has the token.
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-4 flex-1 min-h-0">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={buildToken}
              className={`px-4 rounded-xl text-xs font-bold uppercase tracking-wider ${t.bg} ${t.text} border ${t.border}`}
            >
              Sign
            </button>
          </div>
          ...
          {builtToken && (
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => handleCopy(builtToken)}
                  className="text-xs text-zinc-400 hover:text-white flex items-center gap-1"
                >
                  {copied ? <Check size={12} /> : <Copy size={12} />} {copied ? 'Copied' : 'Copy'}
                </button>
              </div>
              <div className={`bg-black border rounded-xl p-3 font-mono text-xs break-all ${t.border} ${t.text}`}>
                {builtToken}
              </div>
            </div>
          )}
          <p className="text-xs text-zinc-600 leading-relaxed">
            Runs entirely in your browser — the secret and token never leave this component, and this tab is
            intentionally excluded from history so secrets are never persisted, even in memory.
          </p>
        </div>
      )}
    </div>
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-2">
        {history.length === 0 && (
          <p className="text-xs text-zinc-600">
            Conversions from the Text and Hash tools will show up here as you go. JWT secrets are never recorded.
          </p>
        )}
        {history.map((h) => (
          <button
            key={h.id}
            onClick={() => onRestore(h)}
            className="text-left bg-white/5 hover:bg-white/10 rounded-lg p-3 transition-all"
          >
...
export const PayloadMutator = ({ theme }: { theme: AccentTheme }) => {
  ...
  const [restoreSeed, setRestoreSeed] = useState<Record<HistoryTool, string | undefined>>({
    text: undefined,
    hash: undefined,
  })
  const t = THEME_MAP[theme]
    ...
    <div className="flex-1 w-full h-full p-6 flex flex-col font-sans bg-[#0a0a0c]">
      <div className="flex items-center justify-between mb-4">
        ...
        >
          <HistoryIcon size={14} /> History
          {history.length > 0 && (
            <span className="ml-1 text-[10px] bg-white/10 rounded-full px-1.5 py-0.5">{history.length}</span>
          )}
        </button>
      </div>
      ...
      <div className="relative flex-1 min-h-0">
        <div className={tool === 'text' ? 'h-full' : 'hidden'}>
          <TextTool
            key={`text-${restoreNonce.text}`}
            theme={theme}
            initialInput={restoreSeed.text}
            onResult={pushHistory}
          />
        </div>
        <div className={tool === 'hash' ? 'h-full' : 'hidden'}>
          <HashTool
            key={`hash-${restoreNonce.hash}`}
            theme={theme}
            initialInput={restoreSeed.hash}
            onResult={pushHistory}
          />
        </div>

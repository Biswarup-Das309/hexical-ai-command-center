      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-400 max-w-xl">
            Project context, extracted references, and investigation scope for the current session.
          </p>
        </div>
      </div>
          ...
          <div className="space-y-3">
            {targets.length > 0 ? (
              targets.map((target, index) => (
                <div
                  key={index}
                  className="rounded-2xl border border-white/5 bg-white/5 p-3 text-sm text-zinc-200 flex items-center justify-between"
                >
                  <span>{target}</span>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="rounded-2xl bg-white/5 p-3">
                ...
                <p className="text-white text-lg font-semibold">
                  {targets.length > 0 ? `${targets.length * 12}%` : '0%'}
                </p>
              </div>
              <div className="rounded-2xl bg-white/5 p-3">
                <p className="text-zinc-400 text-[10px] uppercase tracking-widest mb-2">Asset Density</p>
                <p className="text-white text-lg font-semibold">
                  {targets.length > 3 ? 'High' : targets.length > 0 ? 'Moderate' : 'Low'}
                </p>
              </div>
              <div className="rounded-2xl bg-white/5 p-3">
          ...
          <div className="bg-[#111116] border border-white/10 rounded-2xl p-6 shadow-inner">
            ...
            <ul className="space-y-2 text-sm text-zinc-300">
              <li className="rounded-2xl bg-white/5 p-3">
                Use <span className="text-white">Code</span> to inspect submitted source context.
              </li>
              <li className="rounded-2xl bg-white/5 p-3">
                Use <span className="text-white">Impact</span> when a result includes a graph.
              </li>
              <li className="rounded-2xl bg-white/5 p-3">
                Open <span className="text-white">Trace Logs</span> to inspect returned evidence.
              </li>
            </ul>
          </div>
        </div>

...
const ThreatNode = ({ data }: { data: any }) => {
  ...
  const config = isEntry 
    ? { icon: Globe, color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/30', glow: 'shadow-blue-900/20' }
    : isVuln 
      ? { icon: Bug, color: 'text-rose-400', bg: 'bg-rose-500/10', border: 'border-rose-500/30', glow: 'shadow-rose-900/20' }
      : isPivot
        ? { icon: GitMerge, color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/30', glow: 'shadow-amber-900/20' }
        : { icon: Target, color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', glow: 'shadow-emerald-900/20' };
          ...
          {selectedNode && (
            <>
              ...
              <div className="p-5 flex-1 overflow-y-auto">
                ...
                <div className="mb-6">
                  <div className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold mb-2">Classification</div>
                  <span className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider border ${
                    selectedNode.data.type === 'vuln' ? 'bg-rose-500/10 text-rose-400 border-rose-500/30' : 

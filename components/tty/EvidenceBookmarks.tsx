...
export function EvidenceBookmarks({
  executionId,
  candidates = [],
  initialBookmarks,
  onBookmarkAdded,
  ...
  useEffect(() => {
    ...
    try {
      setBookmarks(
        parseTTYEvidenceBookmarks(localStorage.getItem(ttyEvidenceStorageKey(executionId)), executionId as never),
      )
    } catch {
      setBookmarks([])
    }
  ...
  const add = (candidate: TTYEvidenceCandidate) => {
    const existing = bookmarks.find(
      (bookmark) => bookmark.sequence === candidate.sequence && bookmark.kind === candidate.kind,
    )
    if (existing) return
    ...
    >
      ...
      <div className="space-y-1.5">
        {bookmarks.map((bookmark) => (
          <div
            key={bookmark.id}
            className="group flex items-start gap-2 rounded border border-amber-400/15 bg-amber-400/5 p-2"
          >
            ...
            <button
              type="button"
              className="shrink-0 p-1 text-zinc-600 hover:text-rose-300"
        ...
        {candidates.map((candidate) => (
          <button
            key={`${candidate.kind}-${candidate.sequence}`}
            type="button"
            className="flex w-full items-center gap-2 rounded border border-dashed border-white/10 px-2 py-1.5 text-left font-mono text-[10px] text-zinc-500 transition hover:border-cyan-400/30 hover:text-cyan-300"
            onClick={() => add(candidate)}
          >
            <Plus className="size-3 shrink-0" />{' '}
            <span className="truncate">
              {candidate.label}: {candidate.excerpt}
            </span>
          </button>
        ))}
        {bookmarks.length === 0 && candidates.length === 0 && (
          <p className="font-mono text-[10px] text-zinc-600">No evidence bookmarked.</p>
        )}
      </div>
    </section>
  )

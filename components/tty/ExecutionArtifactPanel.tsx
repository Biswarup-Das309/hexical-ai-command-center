    ...
    >
      ...
      <div className="space-y-2 font-mono text-[10px]">
        <div className="flex items-center gap-2 text-zinc-400">
          <ShieldCheck className="size-3 text-emerald-300" /> immutable execution history{' '}
          <span className="ml-auto text-emerald-300">on</span>
        </div>
        <div className="flex items-center gap-2 text-zinc-500">
          <GitCompare className="size-3 text-cyan-300" /> execution diff{' '}
          <span className="ml-auto">{terminal ? 'available' : 'pending'}</span>
        </div>
        <div className="flex items-center gap-2 text-zinc-500">
          <RotateCcw className="size-3 text-violet-300" /> rollback checkpoint{' '}
          <span className="ml-auto">{terminal ? 'recorded' : 'tracking'}</span>
        </div>
      </div>
    </section>
  )

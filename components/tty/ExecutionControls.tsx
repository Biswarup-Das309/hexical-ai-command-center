...
export function ExecutionControls({ executionId, state, outputText, onCancel, onRestart, onClear }: ExecutionControlsProps) {
  ...
  const buttonClass = 'inline-flex h-7 items-center gap-1.5 rounded border border-white/10 bg-white/[0.03] px-2 text-[10px] font-mono uppercase tracking-wider text-zinc-400 transition hover:border-cyan-400/40 hover:bg-cyan-400/10 hover:text-cyan-300 disabled:cursor-not-allowed disabled:opacity-40'
  return (
    <div className="flex flex-wrap items-center gap-1.5" aria-label="Execution controls">
      <button type="button" className={buttonClass} onClick={() => void run('cancel', onCancel)} disabled={isTerminal || !onCancel || busy !== null}>

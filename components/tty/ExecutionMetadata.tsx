...
export function ExecutionMetadata({
  ...
  const fields = [
    ...
    [
      'duration',
      execution?.resourceUsage.durationMs === null || execution?.resourceUsage.durationMs === undefined
        ? '—'
        : `${execution.resourceUsage.durationMs}ms`,
    ],
    ['output size', execution ? `${execution.outputSummary.totalBytes} B` : '—'],
    [
      'exit status',
      completion?.payload.exitCode === null || completion?.payload.exitCode === undefined
        ? value(completion?.payload.state)
        : String(completion.payload.exitCode),
    ],
    ['verification', verificationStatus],
    [
      'queue wait',
      execution?.resourceUsage.queueWaitMs === null || execution?.resourceUsage.queueWaitMs === undefined
        ? '—'
        : `${execution.resourceUsage.queueWaitMs}ms`,
    ],
    [
      'startup',
      execution?.resourceUsage.startupMs === null || execution?.resourceUsage.startupMs === undefined
        ? '—'
        : `${execution.resourceUsage.startupMs}ms`,
    ],
  ] as const
  return (
    <section
          ...
          <div key={label} className="flex items-baseline justify-between gap-3 font-mono text-[10px]">
            ...
            <dd
              className={`min-w-0 truncate text-right ${
                label === 'verification' && fieldValue === 'verified' ? 'text-emerald-400' : 'text-zinc-300'
              }`}
              title={fieldValue}
            >
              {fieldValue}

    ...
    <div className="relative flex h-full w-full flex-col border-r border-white/5 bg-[#0a0a0c] text-foreground shadow-[4px_0_24px_rgba(0,0,0,0.5)]">
      ...
      <div className="mb-5 mt-2 px-3">
        <button
          ...
          disabled={loading}
          className={`flex items-center gap-3 rounded-xl border border-cyan-400/20 bg-cyan-400/[0.06] text-cyan-200 transition-all hover:bg-cyan-400/[0.12] disabled:cursor-wait disabled:opacity-60 ${
      ...
      {isOpen && (
        <div className="flex-1 space-y-4 overflow-y-auto px-3 scrollbar-thin scrollbar-thumb-white/10">
          ...
          <div className="space-y-[2px]">
                  ...
                  {editing ? (
                    <div className="p-2">
                      <input
                        autoFocus
                        value={editValue}
                        disabled={renamingId === investigation.investigationId}
                        onChange={(event) => setEditValue(event.target.value)}
                        onBlur={() => void submitRename(investigation.investigationId, editValue)}
                        onKeyDown={(event) => {
                          if (event.key === 'Enter') {
                            event.preventDefault()
                            void submitRename(investigation.investigationId, editValue)
                          }
                          if (event.key === 'Escape') {
                            setEditingId(null)
                            setEditValue('')
                        ...
                        className="w-full rounded border border-cyan-400/30 bg-black/40 px-2 py-1.5 font-mono text-xs text-cyan-100 outline-none disabled:opacity-60"
                      />
                      {renameError && (
                        <p role="alert" className="mt-1 font-mono text-[9px] text-rose-300">
                          {renameError}
                        </p>
                      )}
                    </div>
                    ...
                    <>
                      <button
                        type="button"
                        onClick={() => onSelect(investigation.investigationId)}
                        className="w-full px-3 py-2.5 pr-10 text-left"
                      >
                        <span
                          className={`block truncate font-mono text-xs ${selected ? 'text-cyan-100' : 'text-zinc-300'}`}
                        >
                          {investigation.title}
                        </span>
                        <span className="mt-1 flex items-center gap-2 font-mono text-[9px] uppercase tracking-wider text-zinc-600">
                          <span>{investigation.status}</span>
                          <span>·</span>
                          <span>{formatUpdatedAt(investigation.updatedAt)}</span>
                        </span>
                      </button>
                      <button
                        type="button"
                        data-investigation-menu-trigger
                        onClick={() =>
                          setMenuOpenId((current) =>
                            current === investigation.investigationId ? null : investigation.investigationId,
                          )
                        }
                        aria-label={`Actions for ${investigation.title}`}
                        className="absolute right-2 top-2 rounded p-1 text-zinc-600 opacity-0 transition-opacity hover:bg-white/10 hover:text-zinc-200 group-hover:opacity-100"
                      >
                        <MoreVertical size={14} />
                      </button>
                      {menuOpenId === investigation.investigationId && (
                        <div
                          data-investigation-menu
                          className="absolute right-2 top-9 z-50 w-36 rounded-lg border border-white/10 bg-[#111116] p-1 shadow-2xl"
                        >
                          <button
                            type="button"
                            onClick={() => startRename(investigation)}
                            className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left font-mono text-[10px] text-zinc-400 hover:bg-white/5 hover:text-white"
                          >
                            <Edit2 size={12} />
                            Rename
                          </button>
                          {investigation.status === 'archived' ? (
                            <button
                              ...
                              onClick={() => {
                                setMenuOpenId(null)
                                void onRestore(investigation.investigationId)
                              }}
                              className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left font-mono text-[10px] text-emerald-300 hover:bg-emerald-400/10"
                            >
                            ...
                            <button
                              ...
                              onClick={() => {
                                setMenuOpenId(null)
                                void onArchive(investigation.investigationId)
                              }}
                              className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left font-mono text-[10px] text-amber-300 hover:bg-amber-400/10"
                            >
                              <Archive size={12} />
                              Archive
                            </button>
                          ...
                          <button
                            ...
                            onClick={() => {
                              setMenuOpenId(null)
                              void onDelete(investigation.investigationId)
                            }}
                            className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left font-mono text-[10px] text-rose-300 hover:bg-rose-400/10"
                          >
                            <Trash2 size={12} />
                            Delete
                          </button>
            ...
            {investigations.length === 0 && (
              <div className="rounded-lg border border-dashed border-white/10 px-4 py-6 text-center font-mono text-[10px] leading-relaxed text-zinc-600">
                No investigations yet.
                <br />
                Create one to establish a persistent evidence workspace.
              </div>
            )}
          </div>
          ...
          {nextCursor && (
            <button
              ...
              disabled={loading}
              className="flex w-full items-center justify-center gap-2 rounded border border-white/10 px-3 py-2 font-mono text-[10px] text-zinc-500 hover:border-cyan-400/30 hover:text-cyan-200 disabled:opacity-50"
            >
      ...
      <div className="mt-auto flex flex-col gap-2 border-t border-white/5 bg-[#0a0a0c] p-3">
        {isOpen && (
          <div className="flex items-center gap-2 rounded-lg border border-white/5 bg-white/[0.02] px-2 py-2">
            <UserButton appearance={{ elements: { avatarBox: 'size-7' } }} />
            <div className="min-w-0 flex-1">
              <div className="truncate font-sans text-xs text-zinc-300">{userName}</div>
              <div className="truncate font-mono text-[9px] text-zinc-600">{userEmail}</div>
            </div>
          </div>
        )}
        <div className="flex items-center gap-1">
          <button
            ...
            className="flex flex-1 items-center justify-center gap-2 rounded-lg border border-cyan-500/20 bg-cyan-500/10 px-2 py-2 font-mono text-[9px] uppercase tracking-wider text-cyan-300 hover:bg-cyan-500/20"
          >
            <UserRound size={13} />
            {isOpen ? 'Plan' : ''}
          </button>
          {isOpen && (
            <Link
              ...
              className="rounded-lg border border-white/5 p-2 text-zinc-500 hover:bg-white/5 hover:text-zinc-200"
              aria-label="Settings"
            >
              <Settings size={14} />
            </Link>
          ...
          {isOpen && onSignOut && (
            <button
              type="button"
              onClick={onSignOut}
              className="rounded-lg border border-white/5 p-2 text-zinc-500 hover:bg-white/5 hover:text-rose-300"
              aria-label="Sign out"
            >
              <LogOut size={14} />
            </button>

...
function latestState(events: readonly TTYStreamEvent[]): TTYExecutionState | null {
  return (
    [...events].reverse().find((event): event is Extract<TTYStreamEvent, { type: 'state' }> => event.type === 'state')
      ?.payload.state ?? null
  )
}

function connectionLabel(state: ReturnType<typeof useTTYExecutionStream>['connectionState']): {
...
export function InvestigationWorkspace({
  ...
  const outputText = useMemo(
    () =>
      stream.events
        .filter(
          (event): event is Extract<TTYStreamEvent, { type: 'stdout' | 'stderr' }> =>
  ...
  const completion = stream.events.find(
    (event): event is Extract<TTYStreamEvent, { type: 'completion' }> => event.type === 'completion',
  )
  ...
  const candidates = useMemo<readonly TTYEvidenceCandidate[]>(() => {
    const errors = stream.events
      .filter((event): event is Extract<TTYStreamEvent, { type: 'error' }> => event.type === 'error')
      .slice(-4)
      .map((event) => ({
        sequence: event.sequence,
        lineNumber: null,
        kind: 'error' as const,
        label: event.payload.code,
        excerpt: event.payload.message,
      }))
    const states = stream.events
      .filter((event): event is Extract<TTYStreamEvent, { type: 'state' }> => event.type === 'state')
      .slice(-4)
      .map((event) => ({
        sequence: event.sequence,
        lineNumber: null,
        kind: 'state' as const,
        label: `state ${event.payload.state}`,
        excerpt: new Date(event.timestamp).toLocaleTimeString([], { hour12: false }),
      }))
    const output = matches.slice(0, 4).map((match) => ({
      sequence: match.sequence,
      lineNumber: match.lineNumber,
      kind: 'output' as const,
      label: `line ${match.lineNumber}`,
      excerpt: lines.find((line) => line.lineNumber === match.lineNumber)?.text ?? '',
    }))
  ...
  return (
    <main
      className="hud-grid min-h-screen bg-[#070709] p-3 text-zinc-200 sm:p-4"
      ...
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-[1800px] flex-col gap-3">
        ...
        <div className="grid min-h-0 flex-1 gap-3 lg:grid-cols-[minmax(180px,220px)_minmax(0,1fr)_minmax(230px,290px)]">
          <aside className="order-2 flex min-h-0 flex-col gap-3 lg:order-1">
            <section className="min-h-[120px] rounded-lg border border-white/10 bg-black/20 p-3">
              <div className="mb-3 flex items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-400">
                <GitBranch className="size-3 text-cyan-300" /> Planner
              </div>
              {plannerPanel ?? (
                <p className="font-mono text-[10px] leading-relaxed text-zinc-600">
                  Attach a planner graph to inspect the execution’s approved investigation path.
                </p>
              )}
            </section>
            <section className="min-h-[120px] flex-1 rounded-lg border border-white/10 bg-black/20 p-3">
              <div className="mb-3 flex items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-400">
                <FileSearch className="size-3 text-amber-300" /> Findings
              </div>
              {findingsPanel ?? (
                <p className="font-mono text-[10px] leading-relaxed text-zinc-600">
                  Verified findings will appear here when the investigation attaches evidence.
                </p>
              )}
            </section>
          ...
          <section className="order-1 flex min-h-[560px] min-w-0 flex-col gap-2 lg:order-2">
            <div className="rounded-lg border border-cyan-400/20 bg-cyan-400/[0.03] p-2">
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  disabled={!onExecute || submitting}
                  aria-label="Execution command"
                  placeholder={
                    onExecute ? 'Enter an approved command. Ctrl+Enter runs it.' : 'Execution session is unavailable.'
                  }
                  className="min-h-12 flex-1 resize-y rounded border border-white/10 bg-black/30 p-2 font-mono text-[11px] text-zinc-200 outline-none placeholder:text-zinc-700 disabled:cursor-not-allowed disabled:opacity-60"
                />
                <button
                  type="button"
                  onClick={() => void submit()}
              ...
              {executionError && (
                <p role="alert" className="mt-1 font-mono text-[10px] text-rose-300">
                  {executionError}
                </p>
              )}
            </div>
            <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-white/10 bg-black/30 p-2">
              <div className="flex min-w-0 flex-1 items-center gap-2">
                ...
                <input
                  ...
                  className="min-w-0 flex-1 bg-transparent font-mono text-[11px] text-zinc-200 outline-none placeholder:text-zinc-700"
                />
                <span className="font-mono text-[10px] text-zinc-600">
                  {search ? `${matches.length ? searchIndex + 1 : 0}/${matches.length}` : '—'}
                </span>
                <button
                  type="button"
                  className="px-1 font-mono text-xs text-zinc-500 hover:text-cyan-300"
            ...
            {stream.error && (
              <div
                className="rounded border border-amber-400/20 bg-amber-400/5 px-2 py-1 font-mono text-[10px] text-amber-200"
                role="status"
              >
                {stream.error}
              </div>
            ...
            {stream.error === 'No active execution' ? (
              <div
                role="status"
                className="flex min-h-0 flex-1 items-center justify-center rounded-lg border border-amber-400/20 bg-amber-400/[0.03] font-mono text-xs uppercase tracking-widest text-amber-200"
              >
                No active execution
              </div>
            ) : (

...
export function PersistentInvestigationWorkspace({
  ...
  useEffect(() => {
    ...
    workspace.data?.investigation.investigationId,
    workspace.data?.investigation.ttySessionId,
  ...
  const history = useMemo(
    ...
    [workspace.data?.executions],
  )
  const bookmarks = useMemo(
    () =>
      workspace.data?.bookmarks.filter((bookmark) => bookmark.executionId === activeExecutionId).map(toTTYBookmark) ??
      [],
  ...
  const archiveInvestigation = async () => {
    if (onArchive) await onArchive()
    else await workspace.archive()
  }
  const restoreInvestigation = async () => {
    if (onRestore) await onRestore()
    else await workspace.restore()
  }
  const deleteInvestigation = async () => {
    if (onDelete) await onDelete()
    else await workspace.remove()
  }
  const createInvestigation = async () => {
    if (onNewInvestigation) await onNewInvestigation()
    else await workspace.create()
  }
  ...
  const execute = async (rawInput: string) => {
    ...
    try {
      ...
      const executionId = await workspace.attachExecution({
  ...
  if (!workspace.data) {
    return (
      <main className="min-h-screen bg-[#070709] p-6 text-zinc-200" aria-label="Investigation workspace loading">
        <div className="mx-auto max-w-3xl rounded-lg border border-white/10 bg-black/30 p-6 font-mono text-xs text-zinc-500">
          {workspace.loading ? 'Hydrating investigation…' : workspace.error ?? 'Preparing investigation workspace…'}
        </div>
      </main>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#070709] text-zinc-200">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-black/30 px-4 py-3">
        ...
        <div className="flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-wider">
          <span className={workspace.loading ? 'text-amber-300' : 'text-emerald-300'} aria-live="polite">
            {workspace.loading ? 'hydrating' : 'hydrated'}
          </span>
          ...
          {investigation.status === 'active' ? (
            <button
              type="button"
              onClick={() => void archiveInvestigation()}
              className="rounded border border-white/10 px-2 py-1 text-zinc-400 hover:border-amber-400/40 hover:text-amber-200"
            >
            ...
            <button
              type="button"
              onClick={() => void restoreInvestigation()}
              className="rounded border border-white/10 px-2 py-1 text-zinc-400 hover:border-emerald-400/40 hover:text-emerald-200"
            >
          ...
          <button
            ...
            className="rounded border border-cyan-400/30 px-2 py-1 text-cyan-200 hover:bg-cyan-400/10"
          >
          ...
          <button
            ...
            className="rounded border border-rose-400/20 px-2 py-1 text-rose-300 hover:bg-rose-400/10"
      ...
      <div className="border-b border-white/10 bg-black/20 px-4 py-2 font-mono text-[10px] text-zinc-500">
        <span className="mr-4">executions {investigation.executionCount}</span>
        <span className="mr-4">evidence {investigation.evidenceCount}</span>
        <span className="mr-4">findings {investigation.findingCount}</span>
        <span className={activeSessionId ? 'text-emerald-300' : sessionFailure ? 'text-rose-300' : 'text-amber-300'}>
          {activeSessionId ? 'session attached' : sessionFailure ? 'session unavailable' : 'attaching session'}
        </span>
      </div>
      {!activeSessionId && sessionFailure && (
        <div
          role="alert"
          className="border-b border-rose-400/20 bg-rose-400/[0.04] px-4 py-2 font-mono text-[10px] text-rose-300"
        >
          <span>{sessionFailure.message}</span>
          {sessionFailure.code === 'CAPABILITY_LOCKED' && (
            <span className="ml-2 text-amber-200">Upgrade to Pro to enable the execution workspace.</span>
          )}
          <button
            type="button"
            onClick={() => void workspace.ensureSession()}
            className="ml-3 text-cyan-200 underline underline-offset-2"
          >
            Retry session
          </button>
        </div>
      ...
      {showExecution ? (
        ...
        <section className="mx-auto grid max-w-5xl gap-4 p-6 lg:grid-cols-[minmax(0,1fr)_minmax(260px,360px)]">
          <div className="rounded-lg border border-white/10 bg-black/20 p-4">
            <div className="mb-4 rounded border border-cyan-400/20 bg-cyan-400/[0.03] p-3">
              <div className="mb-2 flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.2em] text-cyan-300">
                <span>Execute in investigation</span>
                {activeSessionId && (
                  <button
                    type="button"
              ...
              <textarea
                ...
                onKeyDown={(event) => {
                  if ((event.ctrlKey || event.metaKey) && event.key === 'Enter') {
                    ...
                    void execute(executionInput)
                  }
                }}
                disabled={!activeSessionId || submittingExecution}
                aria-label="Investigation execution command"
                placeholder={
                  activeSessionId
                    ? 'Enter an approved command. Ctrl+Enter runs it.'
                    : sessionFailure?.message ?? 'Attaching investigation session…'
                }
                className="min-h-20 w-full rounded border border-white/10 bg-black/30 p-2 font-mono text-xs text-zinc-200 outline-none disabled:cursor-not-allowed disabled:opacity-60"
              />
              <button
                type="button"
                onClick={() => void execute(executionInput)}
                disabled={!activeSessionId || submittingExecution || !executionInput.trim()}
                className="mt-2 rounded border border-cyan-400/30 px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider text-cyan-200 hover:bg-cyan-400/10 disabled:opacity-50"
              >
                <Play className="mr-1 inline size-3" />
                {submittingExecution ? 'Queueing' : 'Execute'}
              </button>
              {executionError && (
                <p role="alert" className="mt-2 font-mono text-[10px] text-rose-300">
                  {executionError}
                </p>
              )}
            </div>
            <div className="mb-3 font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-400">
              Investigation timeline
            </div>
            <div className="space-y-2">
              {workspace.data.timeline.map((event) => (
                <div
                  key={event.eventId}
                  className="rounded border border-white/10 px-3 py-2 font-mono text-[10px] text-zinc-500"
                >
                  <span className="mr-2 text-cyan-300">{event.type}</span>
                  <time dateTime={event.occurredAt}>{new Date(event.occurredAt).toLocaleString()}</time>
                  {event.executionId && <span className="ml-2 text-zinc-700">{event.executionId.slice(0, 8)}</span>}
                </div>
              ))}
              {workspace.data.timeline.length === 0 && (
                <p className="text-xs text-zinc-600">No timeline events persisted.</p>
              )}
            </div>
            {workspace.data.nextTimelineCursor && (
              <button
                type="button"
                onClick={() => void workspace.loadMoreTimeline()}
                className="mt-3 font-mono text-[10px] text-cyan-300"
              >
                Load older timeline
              </button>
            )}
          ...
          <aside className="space-y-4">
            <div className="rounded-lg border border-white/10 bg-black/20 p-4">
              <div className="mb-3 flex items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-400">
                <StickyNote className="size-3 text-amber-300" /> Notes
              </div>
              <textarea
                value={note}
                onChange={(event) => setNote(event.target.value)}
                placeholder="Add an investigation note"
                className="min-h-20 w-full rounded border border-white/10 bg-black/20 p-2 font-mono text-xs text-zinc-300 outline-none"
              />
              <button
                type="button"
                onClick={() => void addNote()}
                className="mt-2 rounded border border-amber-400/20 px-2 py-1 font-mono text-[10px] text-amber-200"
              >
                Add note
              </button>
              {noteError && (
              ...
              <div className="mt-3 space-y-2">
                {workspace.data.notes.slice(-8).map((item) =>
                  editingNoteId === item.noteId ? (
                    <div key={item.noteId} className="space-y-2 border-l border-amber-400/30 pl-2">
                      <textarea
                        autoFocus
                        value={editingNoteBody}
                        onChange={(event) => setEditingNoteBody(event.target.value)}
                        className="min-h-16 w-full rounded border border-amber-400/20 bg-black/20 p-2 font-mono text-[10px] text-zinc-300 outline-none"
                      />
                    ...
                    <div key={item.noteId} className="flex items-start gap-2 border-l border-amber-400/30 pl-2">
                      <p className="min-w-0 flex-1 font-mono text-[10px] text-zinc-500">{item.body}</p>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="text-zinc-600 hover:text-amber-200"
            ...
            <div className="rounded-lg border border-white/10 bg-black/20 p-4 font-mono text-[10px] text-zinc-500">
              Executions remain attached to this investigation across refresh, reconnect, and worker restart.
            </div>
          </aside>
        </section>
      )}

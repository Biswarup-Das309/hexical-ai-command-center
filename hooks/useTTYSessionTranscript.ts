...
async function readJson<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new Error(messageFromBody(body, 'The runtime session request failed.'))
  return body as T
}
...
export function useTTYSessionTranscript(sessionId: string | null): UseTTYSessionTranscriptResult {

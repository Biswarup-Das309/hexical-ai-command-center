...
import {
    PlannerScheduler as PlannerSchedulingEngine,
    PlannerSchedule,
    Schedule
} from "./planner/planner-scheduler";

import {
    ResourceManager
} from "./planner/planner-resource";

import {
    PlannerConstraintManager
} from "./planner/planner-constraint";
...
export class AgentSession
    implements
        ...
        Timestamped,
        Versioned,
        Serializable<AgentSessionSnapshot> {
...
export class AgentDiagnostics {
    ...
    validatePhaseConsistency(session: AgentSession): boolean {
        this.checks++;
        const valid = !(
            session.getPhase() === AgentPhase.Completed &&
            session.getExecutionResult() === undefined
        );
        if (!valid) {
            this.failures++;
        }
...
export class AgentManager {
    ...
    private relaySubsystemEvents(): void {
        this.planner.subscribe({
            onEvent: (event, payload) => this.emit(AgentEvent.SubsystemEvent, {
    ...
    analyze(session: AgentSession): AgentAnalysisResult {
        ...
        try {
            ...
            this.emitStage(currentStage, "started", session);
            const { reasoningSession, response, explanation, report } =
                this.runReasoning(request, severity, semantic, impact, behavior, recommendations);
    ...
    ): { reasoningSession: ReasoningSession; response: ReasoningResponse; explanation: Explanation; report: ReasoningReport } {
        const evidence: EvidenceNode[] = [];

        for (const result of severity) {
            evidence.push(this.toEvidenceNode(
    ...
    private emitStage(stage: PipelineStage, status: "started" | "completed", session: AgentSession): void {
        this.emit(
            status === "started" ? AgentEvent.StageStarted : AgentEvent.StageCompleted,
            { sessionId: session.id, stage }
        );
    }
    ...
    plan(session: AgentSession): AgentPlanResult {
        ...
        try {
            ...
            const goals = [
                ...(request.goals ?? []),
                ...this.deriveGoalsFromRecommendations(analysis.recommendations)
            ];
    ...
    async execute(
        ...
        try {
            ...
            const phase =
                summary.phase === ExecutionPhase.Completed ? AgentPhase.Completed :
                summary.phase === ExecutionPhase.Cancelled ? AgentPhase.Cancelled :
                AgentPhase.Failed;

...
function entityReferenceKey(candidate: { readonly type: EvidenceGraphEntityType; readonly canonicalKey: string }): string {
...
export class EvidenceGraphStore {
  ...
  constructor(private readonly redis: EvidenceGraphRedis, private readonly authorization: EvidenceGraphAuthorization, options?: { readonly logger?: EvidenceGraphStoreLogger }) {
  ...
  async upsertObservations(ownerUserId: string, investigationId: InvestigationId, observations: readonly EvidenceGraphObservation[]): Promise<{ readonly entitiesCreated: number; readonly relationshipsCreated: number }> {
    ...
    for (const observation of observations) {
      ...
      const executionCandidate: EvidenceGraphEntityCandidate = { type: 'execution', canonicalKey: observation.executionId, label: `Execution ${observation.executionId.slice(0, 12)}`, value: null, metadata: { parser: 'system' } }
      const candidates = [
        { type: 'investigation' as const, canonicalKey: investigationId, label: context.title, value: context.title, metadata: { status: context.status, parser: 'system' } },
        executionCandidate,
      ...
      for (const candidate of candidates) {
        ...
        const created = await this.upsertEntityInternal(investigationId, candidate, candidate.type === 'investigation' ? null : observation.executionId, observation.timestamp)
        entitiesCreated += created.created ? 1 : 0
      }
      const executionIdEntity = references.get(entityReferenceKey(executionCandidate))!
      const entityCandidates = candidates.filter(candidate => candidate.type !== 'investigation' && candidate.type !== 'execution')
      ...
      for (const candidate of observation.extraction.relationships) {
        ...
        relationshipsCreated += await this.upsertEdgeInternal(investigationId, {
          ...
          dedupeKey: candidate.dedupeKey ?? `${entityReferenceKey(candidate.source)}:${candidate.relationship}:${entityReferenceKey(candidate.target)}`
        })
      }
    }
  ...
  async getProcessedSequences(ownerUserId: string, investigationId: InvestigationId, executionId: string): Promise<ReadonlySet<number> | null> {
    if (!(await this.authorizedInvestigation(ownerUserId, investigationId))) return null
    const members = await this.redis.zrange<string[]>(graphProcessedEventIndexKey(investigationId, executionId), 0, -1, { offset: 0, count: 100_000 })
  ...
  async markProcessedSequence(investigationId: InvestigationId, executionId: string, sequence: number): Promise<void> {
    await this.redis.zadd(graphProcessedEventIndexKey(investigationId, executionId), { score: sequence, member: String(sequence) })
  }
  ...
  async summary(ownerUserId: string, investigationId: InvestigationId): Promise<EvidenceGraphSummary | null> {
    ...
    const [entityCount, relationshipCount, entityCounts, relationshipCounts, lastUpdatedAt] = await Promise.all([
      ...
      Promise.all((['investigation', 'execution', 'host', 'domain', 'ip', 'port', 'service', 'url', 'technology', 'vulnerability', 'finding', 'credential', 'evidence', 'file', 'screenshot', 'note'] as const).map(async type => [type, await this.redis.zcard(graphEntityTypeIndexKey(investigationId, type))] as const)),
      Promise.all((['DISCOVERED', 'RESOLVES_TO', 'EXPOSES', 'RUNS', 'LINKS_TO', 'DETECTED', 'CONFIRMS', 'EVIDENCE_FOR', 'GENERATED_FROM', 'RELATED_TO'] as const).map(async relationship => [relationship, await this.redis.zcard(graphEdgeRelationshipIndexKey(investigationId, relationship))] as const)),
      this.redis.get<string>(this.lastUpdatedKey(investigationId))
    ])
    return { investigation, entityCount, relationshipCount, entitiesByType: Object.fromEntries(entityCounts), relationshipsByType: Object.fromEntries(relationshipCounts), lastUpdatedAt }
  }

  async listEntities(ownerUserId: string, investigationId: InvestigationId, options: { readonly type?: EvidenceGraphEntityType; readonly cursor?: string | null; readonly limit?: number } = {}): Promise<EvidenceGraphEntityPage | null> {
    if (!(await this.authorizedInvestigation(ownerUserId, investigationId))) return null
    const offset = safeCursor(options.cursor)
    const limit = safeLimit(options.limit, MAX_ENTITY_LIMIT)
    const ids = await this.redis.zrange<string[]>(options.type ? graphEntityTypeIndexKey(investigationId, options.type) : graphEntityIndexKey(investigationId), 0, -1, { rev: true, offset, count: limit + 1 })
    const entities = (await Promise.all(ids.map(id => this.readEntity(investigationId, id)))).filter((entity): entity is EvidenceGraphEntity => entity !== null)
    return { entities: entities.slice(0, limit), nextCursor: entities.length > limit ? String(offset + limit) : null }
  }

  async listRelationships(ownerUserId: string, investigationId: InvestigationId, options: { readonly relationship?: EvidenceGraphRelationship; readonly executionId?: string; readonly cursor?: string | null; readonly limit?: number } = {}): Promise<EvidenceGraphRelationshipPage | null> {
    if (!(await this.authorizedInvestigation(ownerUserId, investigationId))) return null
    const offset = safeCursor(options.cursor)
    const limit = safeLimit(options.limit, MAX_RELATIONSHIP_LIMIT)
    const ids = await this.redis.zrange<string[]>(options.executionId ? graphEdgeExecutionIndexKey(investigationId, options.executionId) : options.relationship ? graphEdgeRelationshipIndexKey(investigationId, options.relationship) : graphEdgeIndexKey(investigationId), 0, -1, { rev: true, offset, count: limit + 1 })
    const relationships = (await Promise.all(ids.map(id => this.readEdge(investigationId, id)))).filter((edge): edge is EvidenceGraphEdge => edge !== null).filter(edge => !options.relationship || edge.relationship === options.relationship).filter(edge => !options.executionId || edge.executionId === options.executionId)
    return { relationships: relationships.slice(0, limit), nextCursor: relationships.length > limit ? String(offset + limit) : null }
  }

  async getEntity(ownerUserId: string, investigationId: InvestigationId, entityId: EvidenceGraphEntityId): Promise<EvidenceGraphEntity | null> {
    if (!(await this.authorizedInvestigation(ownerUserId, investigationId))) return null
    return this.readEntity(investigationId, entityId)
  }

  async getConnected(ownerUserId: string, investigationId: InvestigationId, entityId: EvidenceGraphEntityId, options: { readonly cursor?: string | null; readonly limit?: number } = {}): Promise<EvidenceGraphConnectedPage | null> {
    if (!(await this.authorizedInvestigation(ownerUserId, investigationId))) return null
    ...
    const [outgoing, incoming] = await Promise.all([
      this.redis.zrange<string[]>(graphEdgeSourceIndexKey(investigationId, entityId), 0, -1, { rev: true, offset: 0, count: offset + limit + 1 }),
      this.redis.zrange<string[]>(graphEdgeTargetIndexKey(investigationId, entityId), 0, -1, { rev: true, offset: 0, count: offset + limit + 1 })
    ])
    const edgeIds = [...new Set([...outgoing, ...incoming])]
    const relationships = (await Promise.all(edgeIds.map(id => this.readEdge(investigationId, id)))).filter((edge): edge is EvidenceGraphEdge => edge !== null)
    ...
    const entities = (await Promise.all([...new Set(relatedIds)].map(id => this.readEntity(investigationId, id)))).filter((related): related is EvidenceGraphEntity => related !== null)
    return { entity, relationships: page.slice(0, limit), entities, nextCursor: page.length > limit ? String(offset + limit) : null }
  }

  async executionGraph(ownerUserId: string, investigationId: InvestigationId, executionId: string, options: { readonly cursor?: string | null; readonly limit?: number } = {}): Promise<EvidenceGraphRelationshipPage | null> {
    return this.listRelationships(ownerUserId, investigationId, { executionId, cursor: options.cursor, limit: options.limit })
  }

  async investigationGraph(ownerUserId: string, investigationId: InvestigationId): Promise<EvidenceGraphInvestigationView | null> {
  ...
  private async authorizedInvestigation(ownerUserId: string, investigationId: InvestigationId): Promise<Pick<InvestigationRecord, 'investigationId' | 'title' | 'status'> | null> {
    try {
  ...
  private async upsertEntityInternal(investigationId: InvestigationId, candidate: EvidenceGraphEntityCandidate, sourceExecutionId: string | null, timestamp: string): Promise<{ readonly id: EvidenceGraphEntityId; readonly created: boolean }> {
    const id = graphEntityId(investigationId, candidate.type, candidate.canonicalKey)
    ...
    try {
      const result = await this.redis.eval<number>(ENTITY_UPSERT_SCRIPT, [
  ...
  private async upsertEdgeInternal(investigationId: InvestigationId, edge: { readonly source: EvidenceGraphEntityId; readonly target: EvidenceGraphEntityId; readonly relationship: EvidenceGraphRelationship; readonly executionId: string; readonly timestamp: string; readonly confidence: number; readonly metadata: EvidenceGraphMetadata; readonly dedupeKey: string }): Promise<number> {
    const id = graphEdgeId(investigationId, edge.source, edge.target, edge.relationship, edge.executionId, edge.dedupeKey)
    const stored: EvidenceGraphEdge = { id, source: edge.source, target: edge.target, relationship: edge.relationship, executionId: edge.executionId, investigationId, timestamp: edge.timestamp, confidence: clampConfidence(edge.confidence), metadata: edge.metadata }
    try {
      const result = await this.redis.eval<number>(EDGE_UPSERT_SCRIPT, [

...
export interface EvidenceGraphSynchronizationSource {
  ...
  readonly getExecutions: (
    ownerUserId: string,
    investigationId: InvestigationId,
  ) => Promise<readonly InvestigationExecution[]>
...
export class EvidenceGraphSynchronizer {
  constructor(
    ...
    private readonly source: EvidenceGraphSynchronizationSource,
    private readonly extractor = new DeterministicExtractionEngine(),
  ) {}
  ...
  async synchronizeInvestigation(ownerUserId: string, investigationId: InvestigationId): Promise<void> {
    ...
    for (const execution of executions)
      await this.synchronizeExecutionInternal(ownerUserId, investigationId, execution.executionId, false)
  }
  ...
  private async synchronizeExecutionInternal(
    ownerUserId: string,
    investigationId: InvestigationId,
    executionId: string,
    ...
    let batch: Array<{
      ...
      readonly extraction: ReturnType<DeterministicExtractionEngine['extract']>
    }> = []
    const flushBatch = async (): Promise<void> => {
      if (batch.length === 0) return
      await this.store.upsertObservations(
        ownerUserId,
        investigationId,
        batch.map((item) => ({
          investigationId,
    ...
    for (const event of output) {
      ...
      const extraction =
        event.type === 'stdout' || event.type === 'stderr'
          ? this.extractor.extract(executionId, {
              type: event.type,
              text: event.text ?? '',
    ...
    if (output.length === 0 && batch.length === 0) {
      batch.push({
        sequence: 1,
        timestamp: execution.timestamps.updatedAt,

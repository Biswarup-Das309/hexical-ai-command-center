...
export function assertNever(value: never, context?: string): never {
  throw new Error(`Unhandled union member${context ? ` in ${context}` : ''}: ${JSON.stringify(value)}`)
}
...
export const ExecutionProfileSchema = z.enum(['recon', 'swarm', 'audit', 'deep_fuzz', 'passive_recon'])
...
export type ScanRequestValidation = { success: true; data: ScanRequest } | { success: false; error: string }

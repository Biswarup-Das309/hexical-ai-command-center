...
export function buildSingleSystemPrompt(systemCtx: string, provider: Provider, profile: Profile): string {
  const providerHint = provider === 'groq' ? `Prefer fast, concise answers.\n` : `Prefer careful, high-signal answers.\n`;
...
export function buildPromptPayload(
  ...
  const olderDigest =
    olderTurns.length > 0
      ? [
          ...
          ...olderTurns.slice(-8).map((turn, index) => `${index + 1}. ${turn.role}: ${truncateToChars(turn.content, 160)}`),
        ].join('\n')
      : '';
  ...
  const recentContext = recentTurns.map(turn => `${turn.role}: ${truncateToChars(turn.content, perRecentTurn)}`).join('\n');

  const context = truncateToChars([olderDigest, recentContext].filter(Boolean).join('\n\n'), remainingForContext);

  return {
    promptLogic: [`Conversation context (compressed):`, context, `Current request:`, logic].filter(Boolean).join('\n\n'),
    compressedConversation: true,

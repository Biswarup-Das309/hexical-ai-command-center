...
const TIMELINE_SCHEMA = z.union([
  ...
  z
    .object({
      ...
      kind: z.enum(['output', 'error', 'state', 'finding']),
  ...
  return {
    ...
    async list(request: Request): Promise<Response> {
      try {
        ...
        if (limit !== undefined && (!Number.isSafeInteger(limit) || limit < 1 || limit > 50))
          return failure(400, 'INVALID_PAGINATION', 'The investigation page size is invalid.')
        if (cursor !== null && !/^\d+$/.test(cursor))
          return failure(400, 'INVALID_PAGINATION', 'The investigation cursor is invalid.')
        ...
        return json(
          { ok: true, investigations: page.investigations.map(publicRecord), nextCursor: page.nextCursor },
          200,
        )
      } catch {
    ...
    async get(request: Request, rawId: string): Promise<Response> {
      try {
        ...
        if (!investigationId)
          return failure(400, 'INVALID_INVESTIGATION_ID', 'The investigation identifier is invalid.')
        const url = new URL(request.url)
        ...
        if (
          parsedTimelineLimit !== undefined &&
          (!Number.isSafeInteger(parsedTimelineLimit) || parsedTimelineLimit < 1 || parsedTimelineLimit > 100)
        )
          return failure(400, 'INVALID_PAGINATION', 'The timeline page size is invalid.')
        if (
          parsedExecutionLimit !== undefined &&
          (!Number.isSafeInteger(parsedExecutionLimit) || parsedExecutionLimit < 1 || parsedExecutionLimit > 50)
        )
          return failure(400, 'INVALID_PAGINATION', 'The execution page size is invalid.')
        if (timelineCursor !== null && !/^\d+-\d+$/.test(timelineCursor))
          return failure(400, 'INVALID_PAGINATION', 'The timeline cursor is invalid.')
        if (executionCursor !== null && !/^\d+$/.test(executionCursor))
        ...
        const hydration = await resolveCanonicalInvestigation(
          ...
          {
            onMiss: (attempt) =>
              logger.warn('investigation.hydrate_miss_retry', { requestId, investigationId, userId: user, attempt }),
            onRecovered: (attempt) =>
              logger.info('investigation.hydrate_recovered', { requestId, investigationId, userId: user, attempt }),
          },
        )
        if (!hydration) logger.warn('investigation.hydrate_not_found', { requestId, investigationId, userId: user })
        return hydration
          ? json({ ok: true, ...publicHydration(hydration) }, 200)
          : failure(404, 'NOT_FOUND', 'Investigation not found.')
      } catch {
    ...
    async patch(request: Request, rawId: string): Promise<Response> {
      try {
        ...
        if (!parsed.success || Object.keys(parsed.data).length === 0)
          return failure(400, 'INVALID_INPUT', 'The investigation update is invalid.')
        const investigation = await dependencies
          .getStore()
          .patch(user, investigationId, parsed.data as InvestigationPatchInput)
        return investigation
          ? json({ ok: true, investigation: publicRecord(investigation) }, 200)
          : failure(404, 'NOT_FOUND', 'Investigation not found.')
      } catch {
    ...
    async delete(request: Request, rawId: string): Promise<Response> {
      try {
        ...
        const existing = await dependencies
          .getStore()
          .get(user, investigationId, { timelineLimit: 1, executionLimit: 1 })
        ...
        return deleted
          ? json({ ok: true, investigationId, deleted: true }, 200)
          : failure(404, 'NOT_FOUND', 'Investigation not found.')
      } catch {
...
function sessionFromResponse(body: unknown): { readonly sessionId: string; readonly status: string } | null {
  ...
  if (typeof session !== 'object' || session === null || !('sessionId' in session) || !('status' in session))
    return null
  return typeof session.sessionId === 'string' && typeof session.status === 'string'
    ...
    async ensure(request: Request, rawId: string): Promise<Response> {
      ...
      try {
        ...
        const hydration = await resolveCanonicalInvestigation(
          ...
          {
            onMiss: (attempt) =>
              logger.warn('investigation.session_resolve_miss_retry', {
                requestId,
                investigationId,
                userId: user,
                attempt,
            ...
            onRecovered: (attempt) =>
              logger.info('investigation.session_resolve_recovered', {
                requestId,
                investigationId,
                userId: user,
                attempt,
        ...
        if (!hydration) {
          ...
          const reason = !diagnosis.present
            ? 'absent'
            : !diagnosis.ownerMatches
            ? 'owner_mismatch'
            : diagnosis.status === 'deleted'
            ? 'deleted'
            : 'owner_matches_active_but_unresolved'
        ...
        if (existingSessionId) {
          logger.info('investigation.session_ensure_existing_reference', {
            requestId,
            investigationId,
            userId: user,
          ...
          logger.warn('investigation.session_ensure_stale_reference_repaired', {
            requestId,
            investigationId,
            userId: user,
            sessionId: existingSessionId,
        ...
        if (!createdResponse.ok || !created) {
          logger.error('investigation.session_ensure_create_failed', {
            requestId,
            investigationId,
            userId: user,
          ...
          return json(
            createdBody ?? {
              ...
              message: 'The execution session could not be created.',
            },
        ...
        if (!attached) {
          logger.error('investigation.session_ensure_attach_lost_ownership', {
            requestId,
            investigationId,
            userId: user,
        ...
        if (attached.ttySessionId !== created.sessionId) {
          ...
          logger.info('investigation.session_ensure_concurrent_create_deduped', {
            requestId,
            investigationId,
            userId: user,
            sessionId: attached.ttySessionId,
            discardedSessionId: created.sessionId,
          })
          await dependencies.terminateTTYSession(emptyRequest(request.url, 'DELETE'), created.sessionId).catch(() => {})
          return json(
            { ok: true, investigation: publicRecord(attached), sessionId: attached.ttySessionId, reused: true },
            200,
          )
        }
        logger.info('investigation.session_ensure_created', {
          requestId,
          investigationId,
          userId: user,
          sessionId: created.sessionId,
        ...
        return json(
          { ok: true, investigation: publicRecord(attached), sessionId: created.sessionId, reused: false },
          201,
        )
      } catch (error) {
        logger.error('investigation.session_ensure_internal_error', {
          requestId,
          investigationId: rawId,
    ...
    async terminate(request: Request, rawId: string): Promise<Response> {
      ...
      try {
        ...
        if (!investigationId)
          return failure(400, 'INVALID_INVESTIGATION_ID', 'The investigation identifier is invalid.')
        const store = dependencies.getStore()
        const hydration = await resolveCanonicalInvestigation(store, user, investigationId, {
          timelineLimit: 1,
          executionLimit: 1,
        })
        if (!hydration) {
        ...
        if (!sessionId)
          return json({ ok: true, investigation: publicRecord(hydration.investigation), terminated: false }, 200)
        const terminatedResponse = await dependencies.terminateTTYSession(
          emptyRequest(request.url, 'DELETE'),
          sessionId,
        )
        if (!terminatedResponse.ok) {
          const body: unknown = await terminatedResponse.json().catch(() => null)
          logger.error('investigation.session_terminate_failed', {
            requestId,
            investigationId,
            userId: user,
            sessionId,
            status: terminatedResponse.status,
          })
          return json(
            body ?? {
              ok: false,
              code: 'SESSION_NOT_TERMINATED',
              message: 'The execution session could not be terminated.',
            },
            terminatedResponse.status,
          )
        }
        ...
        return investigation
          ? json({ ok: true, investigation: publicRecord(investigation), terminated: true }, 200)
          : failure(404, 'NOT_FOUND', 'Investigation not found.')
      } catch (error) {
        logger.error('investigation.session_terminate_internal_error', {
          requestId,
          investigationId: rawId,
          message: error instanceof Error ? error.message : String(error),
        })
  ...
  return {
    async attach(request: Request, rawId: string): Promise<Response> {
      ...
      try {
        ...
        const investigation = await resolveCanonicalInvestigation(dependencies.getStore(), user, investigationId, {
          executionLimit: 1,
          timelineLimit: 1,
        })
        ...
        if (dependencies.ensureSession) {
          ...
          if (ensured.status < 200 || ensured.status >= 300) {
            logger.warn('investigation.execution_session_ensure_failed', {
              requestId,
              investigationId,
              userId: user,
            ...
            return json(
              ensuredBody ?? {
                ...
                message: 'The execution session could not be attached.',
              },
              ensured.status,
            )
          }
          const ensuredSessionId =
            typeof ensuredBody === 'object' &&
            ensuredBody !== null &&
            'sessionId' in ensuredBody &&
            typeof ensuredBody.sessionId === 'string'
              ? ensuredBody.sessionId
              : null
        ...
        const admissionCode = () =>
          typeof admittedBody === 'object' &&
          admittedBody !== null &&
          'code' in admittedBody &&
          typeof admittedBody.code === 'string'
            ? admittedBody.code
            : null
        if (
          (admitted.status === 404 || admitted.status === 409) &&
          dependencies.ensureSession &&
          (admissionCode() === 'SESSION_NOT_FOUND' || admissionCode() === 'SESSION_TERMINATED')
        ) {
          const rebound = await dependencies.ensureSession(
            new Request(request.url, { method: 'POST' }),
            investigationId,
          )
          ...
          const reboundSessionId =
            rebound.status >= 200 &&
            rebound.status < 300 &&
            typeof reboundBody === 'object' &&
            reboundBody !== null &&
            'sessionId' in reboundBody &&
            typeof reboundBody.sessionId === 'string'
              ? reboundBody.sessionId
              : null
          if (!reboundSessionId)
            return json(
              reboundBody ?? {
                ok: false,
                code: 'SESSION_UNAVAILABLE',
                message: 'The execution session could not be restored.',
              },
              rebound.status,
            )
        ...
        if (admitted.status < 200 || admitted.status >= 300) {
          ...
          logger.info('investigation.execution_attach_phases', {
            requestId,
            investigationId,
            userId: user,
            resolve: resolvePhaseMs,
            admit: admitPhaseMs,
            attach: admitPhaseMs,
            activate: admitPhaseMs,
            totalMs: admitPhaseMs,
          })
          return json(
            admittedBody ?? { ok: false, code: 'EXECUTION_NOT_ADMITTED', message: 'The execution was not admitted.' },
            admitted.status,
          )
        }
        if (typeof admittedBody !== 'object' || admittedBody === null)
          return failure(502, 'INVALID_ADMISSION_RESPONSE', 'The execution admission response was invalid.')
        const body = admittedBody as {
          ok?: boolean
          duplicate?: boolean
          job?: { executionId?: string; sessionId?: string }
        }
        if (!body.ok || !body.job?.executionId || body.job.sessionId !== sessionId)
          return failure(502, 'INVALID_ADMISSION_RESPONSE', 'The execution admission response was invalid.')
        const admitPhaseMs = elapsed()
        const attached = await dependencies
          .getStore()
          .attachExecution(user, investigationId, { executionId: body.job.executionId, sessionId })
        ...
        if (dependencies.startExecution) {
          ...
          const result = await raceActivationBudget(
            ...
            {
              onRequested: () =>
                logger.info('investigation.execution_activation_requested', {
                  requestId,
                  investigationId,
                  userId: user,
                  executionId,
              ...
              onPending: (budget) =>
                logger.info('investigation.execution_activation_pending', {
                  requestId,
                  investigationId,
                  userId: user,
                  executionId,
              ...
              onSettledLate: (settled) =>
                logger.info('investigation.execution_activation_settled_late', {
                  requestId,
                  investigationId,
                  userId: user,
                  executionId,
              ...
              onErroredLate: (message) =>
                logger.error('investigation.execution_activation_errored_late', {
                  requestId,
                  investigationId,
                  userId: user,
                  executionId,
              ...
              onRejected: (reason) =>
                logger.warn('investigation.execution_activation_rejected', {
                  requestId,
                  investigationId,
                  userId: user,
                  executionId,
              ...
              onAccepted: () =>
                logger.info('investigation.execution_activation_accepted', {
                  requestId,
                  investigationId,
                  userId: user,
                  executionId,
          ...
          if (result.kind === 'pending') {
            logger.info('investigation.execution_attach_phases', {
              ...
              resolve: resolvePhaseMs,
              admit: admitPhaseMs,
              attach: attachPhaseMs,
              activate: activatePhaseMs,
              totalMs: activatePhaseMs,
            })
            return json(
              {
                ok: true,
          ...
          if (result.kind === 'rejected')
            return failure(503, 'EXECUTION_NOT_STARTED', 'The execution could not be started.')
          logger.info('investigation.execution_attach_phases', {
            requestId,
            investigationId,
            userId: user,
          ...
          return json(
            { ok: true, investigationId, execution: attached, job: body.job, duplicate: body.duplicate === true },
            admitted.status,
          )
        }
        const activatePhaseMs = elapsed()
        ...
        logger.error('investigation.execution_attach_internal_error', {
          requestId,
          investigationId: rawId,
          message: error instanceof Error ? error.message : String(error),
        })

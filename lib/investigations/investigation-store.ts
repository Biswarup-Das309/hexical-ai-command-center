...
): { readonly streamId: string; readonly event: InvestigationTimelineEvent } | null {
...
function parseTimelineEntries(
  raw: unknown,
  investigationId: InvestigationId,
): readonly { readonly streamId: string; readonly event: InvestigationTimelineEvent }[] {
  if (!Array.isArray(raw)) return []
  return raw
    .map((value) => parseTimelineEntry(value, investigationId))
    .filter(
      (value): value is { readonly streamId: string; readonly event: InvestigationTimelineEvent } => value !== null,
    )
}
...
export class InvestigationStore {
  ...
  constructor(
    private readonly redis: InvestigationRedis,
    options?: { readonly logger?: InvestigationStoreLogger },
  ) {
  ...
  async create(
    ...
    try {
      ...
      await this.appendTimeline(
        ...
        'investigation_created',
        { title: input.title },
        { dedupeKey: `created:${investigationId}`, occurredAt: now },
      )
      return record
    } catch (error) {
  ...
  async list(
    ...
    while (true) {
      const ids = await this.redis.zrange<string[]>(investigationOwnerIndexKey(ownerUserId), 0, -1, {
        rev: true,
        offset: scanOffset,
        count: limit + 1,
      })
  ...
  async get(
    ...
    const [executionIds, timelineRaw, bookmarkRaw] = await Promise.all([
      this.redis.zrange<string[]>(investigationExecutionIndexKey(investigationId), 0, -1, {
        rev: true,
        offset: executionOffset,
        count: executionLimit + 1,
      }),
      this.redis.xrange(
        investigationTimelineKey(investigationId),
        options.timelineCursor ? `(${options.timelineCursor}` : '-',
        '+',
        timelineLimit + 1,
      ),
      this.redis.zrange<string[]>(investigationBookmarkIndexKey(investigationId), 0, -1, {
        rev: true,
        offset: 0,
        count: 500,
      }),
    ...
    const executions = await Promise.all(
      executionIds.map((executionId) => this.readExecution(investigationId, executionId)),
    )
    const parsedTimeline = parseTimelineEntries(timelineRaw, investigationId)
    const visibleExecutions = executions
      .filter((execution): execution is InvestigationExecution => execution !== null)
    ...
    return {
      ...
      bookmarks: bookmarkRaw
        .map((value) => parseJson<InvestigationBookmark>(value))
        .filter((value): value is InvestigationBookmark => value !== null),
  ...
  async patch(
    ...
    if (next.title !== current.title) {
      await this.appendTimeline(
        ...
        'investigation_renamed',
        { title: next.title },
        { dedupeKey: `rename:${now}:${next.title}`, occurredAt: now },
      )
    }
    if (next.status !== current.status) {
      await this.appendTimeline(
        investigationId,
        next.status === 'archived' ? 'investigation_archived' : 'investigation_unarchived',
        {},
        { dedupeKey: `status:${now}:${next.status}`, occurredAt: now },
      )
    }
  ...
  async delete(
    ...
    const deleted: InvestigationRecord = {
      ...current,
      status: 'deleted',
      updatedAt: now,
    ...
    await this.appendTimeline(
      ...
      'investigation_deleted',
      {},
      { dedupeKey: `deleted:${now}`, occurredAt: now },
    )
    await this.redis.zrem(investigationOwnerIndexKey(ownerUserId), investigationId)
  ...
  async attachSession(
    ...
    await this.appendTimeline(
      ...
      { dedupeKey: `session-attached:${persistedSessionId}`, occurredAt: now },
    )
    return next
  }
  ...
  async clearSession(
    ...
    await this.appendTimeline(
      ...
      'session_terminated',
      { sessionId },
      { dedupeKey: `session-terminated:${sessionId}`, occurredAt: now },
    )
    return next
  }
  ...
  async attachExecution(
    ownerUserId: string,
    investigationId: InvestigationId,
    input: InvestigationExecutionAttachmentInput,
    ...
    await Promise.all([
      this.redis.zadd(investigationExecutionIndexKey(investigationId), {
        score: Date.parse(now),
        member: input.executionId,
      }),
    ...
    await this.appendTimeline(
      ...
      'execution_queued',
      { sessionId: input.sessionId },
      { executionId: input.executionId, dedupeKey: `execution-queued:${input.executionId}`, occurredAt: now },
    )
    return execution
  }
  ...
  async updateExecution(
    ...
    const eventType: InvestigationTimelineEventType =
    ...
    const dedupeKey =
      eventType === 'execution_started'
        ...
        ? `execution-terminal:${executionId}:${state}`
        : state === 'queued'
        ? `execution-queued:${executionId}`
        : `execution-state:${executionId}:${state}:${updatedAt}`
  ...
  async recordBookmark(
    ownerUserId: string,
    investigationId: InvestigationId,
    bookmark: Omit<InvestigationBookmark, 'bookmarkId' | 'createdAt'>,
    now = new Date().toISOString(),
  ): Promise<InvestigationTimelineEvent | null> {
    if (!(await this.readRecord(investigationId, ownerUserId))) return null
    const bookmarkId = crypto.randomUUID()
    const event = await this.appendTimeline(
      investigationId,
      'evidence_bookmarked',
      { bookmarkId, ...bookmark, createdAt: now },
      {
        executionId: bookmark.executionId,
        dedupeKey: `bookmark:${bookmark.executionId}:${bookmark.sequence}:${bookmark.kind}`,
        occurredAt: now,
      },
    )
    if (event) {
      await this.redis.zadd(investigationBookmarkIndexKey(investigationId), {
        score: Date.parse(now),
        member: JSON.stringify({ bookmarkId, ...bookmark, createdAt: now }),
      })
  ...
  async recordNote(
    ...
    return this.appendTimeline(
      investigationId,
      'note_added',
      { noteId, body },
      { dedupeKey: `note:${noteId}`, occurredAt: now },
    )
  }
  ...
  async updateNote(
    ...
    return this.appendTimeline(
      ...
      'note_edited',
      { noteId, body },
      { dedupeKey: `note-edit:${noteId}:${now}:${body}`, occurredAt: now },
    )
  }
  ...
  async deleteNote(
    ...
    return this.appendTimeline(
      ...
      'note_deleted',
      { noteId },
      { dedupeKey: `note-delete:${noteId}:${now}`, occurredAt: now },
    )
  }
  ...
  async recordExecutionEvent(
    ...
    event: {
      readonly type: Extract<
        InvestigationTimelineEventType,
        'execution_started' | 'stdout' | 'stderr' | 'execution_completed' | 'execution_failed'
      >
      readonly executionId: string
    ...
    const dedupeKey =
      event.type === 'execution_started'
        ? `execution-started:${event.executionId}`
        : event.type === 'execution_completed' || event.type === 'execution_failed'
        ? `execution-terminal:${event.executionId}:${String(event.payload?.state ?? event.type)}`
        : `execution-event:${event.executionId}:${event.sequence ?? occurredAt}:${event.type}`
    return this.appendTimeline(investigationId, event.type, event.payload ?? {}, {
  ...
  private async readRecord(investigationId: InvestigationId, ownerUserId: string): Promise<InvestigationRecord | null> {
    ...
    return Object.freeze({
      ...raw,
      ttySessionId: typeof raw.ttySessionId === 'string' ? raw.ttySessionId : null,
      executionCount: counterValue(executions ?? raw.executionCount),
      evidenceCount: counterValue(evidence ?? raw.evidenceCount),
      findingCount: counterValue(findings ?? raw.findingCount),
    })
  }
  ...
  private async readExecution(
    ...
    return parseJson<InvestigationExecution>(
      await this.redis.get(investigationExecutionKey(investigationId, executionId)),
    )
  }
  ...
  private async findNote(
    ...
    return (
      this.notesFromTimeline(parseTimelineEntries(raw, investigationId).map((entry) => entry.event)).find(
   ...
   */
  async diagnoseAbsence(
    investigationId: InvestigationId,
    ownerUserId: string,
  ): Promise<{
    readonly present: boolean
    readonly ownerMatches: boolean
    readonly status: InvestigationRecord['status'] | null
  }> {
    const raw = parseJson<InvestigationRecord>(await this.redis.get(investigationRecordKey(investigationId)))
    return raw
      ? { present: true, ownerMatches: raw.ownerUserId === ownerUserId, status: raw.status }
      : { present: false, ownerMatches: false, status: null }
  }
}

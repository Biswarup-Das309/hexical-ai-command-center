...
import { ReasoningSession, ReasoningReport } from './reasoner'
...
export interface MemoryObject extends Identifiable, Timestamped, Versioned, Validatable {}
...
export class MemoryNode<T = unknown>
  implements Serializable<MemoryEntry<T>>, Cloneable<MemoryNode<T>>, Validatable, Versioned, Identifiable
{
  ...
  isMutable(): boolean {
    return !(this.deleted || this.archived || this.frozen || this.state === MemoryState.Locked)
  }

  isUsable(): boolean {
    return !this.deleted && !this.archived && this.state === MemoryState.Active
  }
  ...
  hasGraphConnections(): boolean {
    return this.hasParent() || this.hasChildren() || this.hasRelations() || this.hasDependencies()
  }
  ...
  removeChild(id: MemoryId): this {
    this.assertMutable()
    this.relationships.childIds = this.relationships.childIds.filter((child) => child !== id)
  ...
  merge(other: MemoryNode<T>): this {
    ...
    this.metadata.tags = [...new Set([...this.metadata.tags, ...other.getTags()])]

    this.metadata.labels = [...new Set([...this.metadata.labels, ...other.getLabels()])]

    this.metadata.confidence = Math.max(this.metadata.confidence, other.getConfidence())

    this.metadata.importance = Math.max(this.metadata.importance, other.getImportance())
...
export class MemoryStore<T = unknown> {
  ...
  reduce<R>(callback: (accumulator: R, node: MemoryNode<T>, id: MemoryId) => R, initialValue: R): R {
...
export class MemorySnapshotManager<T = unknown> {
  ...
  snapshotsArray(): MemorySnapshot<T>[] {
    return this.history.map((id) => this.snapshots.get(id)!).map((snapshot) => structuredClone(snapshot))
  }
...
export class MemoryRankingEngine<T = unknown> {
  ...
  private importanceWeight = 0.3
  private recencyWeight = 0.2
...
export class MemoryConsolidator<T = unknown> {
  ...
  similarity(first: MemoryNode<T>, second: MemoryNode<T>): number {
    ...
    score += (sharedLabels / maxLabels) * 0.3
...
export class MemoryManager<T = unknown> {
  ...
  validate(): boolean {
    return this.diagnostics.runAll(this.store, this.index, this.graph, this.cache, this.snapshots, this.archive)
  }

...
import { PlannerSchedule } from './planner-scheduler'
import {
  Dictionary,
  JsonValue,
  Optional,
...
>([
  ...
  [
    ExecutionState.Failed,
    new Set([ExecutionState.Retrying, ExecutionState.Skipped, ExecutionState.RolledBack, ExecutionState.Cancelled]),
  ],
  [
    ExecutionState.TimedOut,
    new Set([ExecutionState.Retrying, ExecutionState.Skipped, ExecutionState.RolledBack, ExecutionState.Cancelled]),
...
export class ExecutionTask
  implements Serializable<ExecutionRecord>, Cloneable<ExecutionTask>, Validatable, Versioned, Identifiable, Timestamped
{
  ...
  setState(state: ExecutionState, reason?: string): this {
    ...
    if (!allowed.has(state)) {
      throw new Error(`Illegal execution state transition for task '${this.id}': '${this.state}' -> '${state}'.`)
    }
  ...
  describe(): string {
    return [`ExecutionTask(${this.id})`, `state=${this.state}`, `attempt=${this.attempt}/${this.maxAttempts}`].join(
      ', ',
    )
  }
...
export class ExecutionDiagnostics {
  ...
  validateTerminalConsistency(context: ExecutionContext): boolean {
    this.checks++
    const valid =
      context.getPhase() !== ExecutionPhase.Completed || context.allTasks().every((task) => task.isTerminal())
...
export class ExecutionContext {
  ...
  setPhase(phase: ExecutionPhase): void {
    this.phase = phase
    if (phase === ExecutionPhase.Completed || phase === ExecutionPhase.Failed || phase === ExecutionPhase.Cancelled) {
  ...
  describe(): string {
    return [`ExecutionContext(${this.planId})`, `phase=${this.phase}`, `tasks=${this.executionTasksById.size}`].join(
      ', ',
    )
  }
...
export class ExecutorManager {
  ...
  async rollback(planId: PlanId, plan: RollbackPlan, handler: TaskExecutionHandler): Promise<TaskId[]> {
    ...
    const rolledBack = await this.rollbackManager.execute(plan, tasksById, definitionsById, handler, () =>
      context.isCancelled(),
    )

...
export class PlannerConstraint
  implements Serializable<Constraint>, Cloneable<PlannerConstraint>, Validatable, Versioned, Identifiable, Timestamped
{
...
export class PlannerConstraintEvaluator {
  ...
  evaluateGoal(goal: PlannerGoal, constraints: readonly PlannerConstraint[]): ValidationIssue[] {
    const issues: ValidationIssue[] = []
  ...
  describe(): string {
    return ['PlannerConstraintEvaluator', `evaluations=${this.evaluations}`, `violations=${this.violations}`].join(', ')
  }
...
export class PlannerConstraintDiagnostics {
  ...
  runAll(
    ...
    return this.validateConstraintInvariants(manager) && this.validateReferences(manager, tasksById, goalsById)
  }
...
export class PlannerConstraintManager {
  ...
  runDiagnostics(tasksById: ReadonlyMap<TaskId, PlannerTask>, goalsById: ReadonlyMap<GoalId, PlannerGoal>): boolean {

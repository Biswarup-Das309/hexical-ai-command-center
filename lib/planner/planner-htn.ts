...
export class HTNMethodRegistry {
  ...
  register(method: HTNMethod): void {
    ...
    for (const subtask of method.subtasks) {
      ...
      if (subtask.type === HTNNodeType.Compound && subtask.goalType === undefined) {
        throw new Error(`HTN method '${method.id}' subtask '${subtask.id}' is Compound but has no goalType.`)
      }
    }
  ...
  methodsFor(goalType: GoalType): HTNMethod[] {
    ...
    return [...ids].map((id) => this.methods.get(id)!).sort((a, b) => b.priority - a.priority)
  }
...
export class HTNPlanner implements GoalDecomposer {
  ...
  private expandGoal(
    ...
    for (const template of method.subtasks) {
      ...
      if (template.type === HTNNodeType.Primitive) {
        ...
        children.push({
          id: generateId('htn-node'),
          type: HTNNodeType.Primitive,
          goalId: goal.id,
          taskId: task.id,
          depth: depth + 1,
          children: [],
  ...
  private instantiateTask(template: HTNSubtaskTemplate, goal: Goal, context: PlannerContext): Task {
    ...
    return {
      ...
      dependencyIds: [],
      resourceRequirements: template.resourceRequirements ? structuredClone(template.resourceRequirements) : [],
      constraintIds: [],
      retryCount: 0,

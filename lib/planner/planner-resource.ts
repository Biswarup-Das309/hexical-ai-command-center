...
import { ResourceId, ResourceState, ResourceRequirement, TaskId, RiskLevel, generateId } from './planner'
import {
  Dictionary,
  Optional,
  Predicate,
...
export class PlannerResource
  implements Serializable<Resource>, Cloneable<PlannerResource>, Validatable, Versioned, Identifiable, Timestamped
{
  ...
  reserveAmount(amount: number): this {
    ...
    if (!this.hasCapacityFor(amount)) {
      throw new Error(`Resource '${this.id}' cannot reserve ${amount}: only ${this.getAvailable()} available.`)
    }
  ...
  allocateAmount(amount: number): this {
    ...
    if (!this.hasCapacityFor(amount)) {
      throw new Error(`Resource '${this.id}' cannot allocate ${amount}: only ${this.getAvailable()} available.`)
    }
  ...
  commitReservedAmount(amount: number): this {
    ...
    if (amount > this.reserved) {
      throw new Error(`Resource '${this.id}' cannot commit ${amount}: only ${this.reserved} currently reserved.`)
    }
    this.reserved -= amount
    this.allocated += amount
    this.touch()
    ...
    Timestamped
{
  ...
  describe(): string {
    return [`PlannerResourcePool(${this.id})`, `name=${this.name}`, `resources=${this.resourceIds.length}`].join(', ')
  }
...
export class ResourceConflictDetector {
  ...
  detectResourceConflicts(resource: PlannerResource): ResourceConflict[] {
    ...
    if (resource.getState() === ResourceState.Available && resource.getAvailable() <= 0) {
      ...
      conflicts.push({
        ...
        resourceId: resource.id,
        description: `Resource '${resource.id}' is marked '${ResourceState.Available}' but has no available capacity.`,
    ...
    if (resource.getState() === ResourceState.Unavailable && resource.getAllocated() > 0) {
      ...
      conflicts.push({
        ...
        type: ResourceConflictType.StateMismatch,
        resourceId: resource.id,
        description: `Resource '${resource.id}' is marked '${ResourceState.Unavailable}' but still has active allocations.`,
  ...
  detectPoolConflicts(pool: PlannerResourcePool, knownResourceIds: ReadonlySet<ResourceId>): ResourceConflict[] {
    const conflicts: ResourceConflict[] = []
  ...
  detectAll(resources: Iterable<PlannerResource>, pools: Iterable<PlannerResourcePool>): ResourceConflict[] {
    const conflicts: ResourceConflict[] = []
...
export class ResourceReservationLedger {
   ...
   */
  expireDue(
    resourcesById: ReadonlyMap<ResourceId, PlannerResource>,
  ...
  allocateDirect(resource: PlannerResource, request: { amount: number; taskId?: TaskId }): ResourceAllocation {
    if (request.amount <= MIN_RESOURCE_AMOUNT) {
      throw new RangeError('Allocation amount must be greater than zero.')
    }
    if (!resource.hasCapacityFor(request.amount)) {
...
export class ResourceDiagnostics {
   ...
   */
  findOrphanedRecords(manager: ResourceManager): {
    reservationIds: ResourceReservationId[]
    allocationIds: ResourceAllocationId[]
  } {
    this.checks++
...
export class ResourceManager {
  ...
  removeResource(id: ResourceId, force = false): boolean {
    ...
    if (!force) {
      const hasActiveReservations = this.ledger
        .reservationsForResource(id)
        .some((reservation) => reservation.status === ResourceReservationStatus.Active)
      const hasActiveAllocations = this.ledger
        .allocationsForResource(id)
        .some((allocation) => allocation.releasedAt === undefined)

      if (hasActiveReservations || hasActiveAllocations) {
        throw new Error(`Resource '${id}' has active reservations or allocations; pass force=true to remove anyway.`)
      }
    }

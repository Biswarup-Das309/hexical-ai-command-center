...
export class PlannerSchedule
    ...
    constructor(schedule: Schedule) {
        ...
        this.entriesById = new Map(
            schedule.entries.map(entry => [entry.taskId, new PlannerScheduleEntry(entry)])
        );
...
export class PlannerConcurrencyAllocator {
    ...
    allocate(
        ...
        const slots = new BinaryMinHeap<{ slot: number; availableAt: number }>((a, b) =>
            a.availableAt - b.availableAt || a.slot - b.slot
        );
...
export class PlannerSchedulePropagationEngine {
    ...
    propagateShift(
        ...
        const duration = tasksById.get(taskId)?.getEstimatedDurationMs()
            ?? nonNegative(entry.getActualFinish() - entry.getActualStart());
        ...
        while (queue.length > 0) {
            ...
            if (requiredStart > currentEntry.getActualStart()) {
                const currentDuration = tasksById.get(current)?.getEstimatedDurationMs()
...
export class PlannerScheduleConflictDetector {
    ...
    detectAll(
        ...
        conflicts.push(...this.detectConcurrencyViolations(schedule, options.maxConcurrency ?? schedule.getMaxConcurrency()));
...
export class PlannerScheduler {
    ...
    reschedule(
        ...
        const rebuilt = this.buildScheduleInternal(
            existing.planId, tasks, graph, policy, maxConcurrency, referenceStart, scheduleId
        );
    ...
    optimizeSchedule(
        ...
        const maxConcurrency = options.maxConcurrency ?? this.defaultMaxConcurrency;
        const referenceStart = options.referenceStart ?? Date.now();
        const policies = candidatePolicies.length > 0
            ? candidatePolicies
            : (Object.values(SchedulePolicy) as SchedulePolicy[]);

...
import { ReasoningReport } from '../reasoner'
...
export class PlannerNode implements Serializable<PlanNode>, Cloneable<PlannerNode>, Validatable, Identifiable {
  ...
  describe(): string {
    return [`PlannerNode(${this.id})`, `task=${this.taskId}`, `children=${this.childIds.length}`].join(', ')
  }
...
export class PlannerGoal
  implements Serializable<Goal>, Cloneable<PlannerGoal>, Validatable, Versioned, Identifiable, Timestamped
{
  ...
  isTerminal(): boolean {
    return (
      this.status === GoalStatus.Achieved || this.status === GoalStatus.Failed || this.status === GoalStatus.Abandoned
    )
  }
  ...
  validate(): boolean {
    return this.id.length > 0 && this.title.length > 0 && this.successCriteria.length >= 0
  }
  ...
  describe(): string {
    return [`PlannerGoal(${this.id})`, `type=${this.type}`, `status=${this.status}`, `priority=${this.priority}`].join(
      ', ',
    )
  }
...
export class PlannerTask
  implements Serializable<Task>, Cloneable<PlannerTask>, Validatable, Versioned, Identifiable, Timestamped
{
  ...
  describe(): string {
    return [`PlannerTask(${this.id})`, `name=${this.name}`, `state=${this.state}`, `priority=${this.priority}`].join(
      ', ',
    )
  }
...
export class PlannerConstraintEngine {
  ...
  private isSatisfied(constraint: Constraint, task: PlannerTask): boolean {
    switch (constraint.type) {
      ...
      case ConstraintType.Resource:
        return task.getResourceRequirements().every((requirement) => requirement.state !== ResourceState.Unavailable)
  ...
  describe(): string {
    return ['PlannerConstraintEngine', `evaluations=${this.evaluations}`, `violations=${this.violations}`].join(', ')
  }
...
export class PlannerScheduler {
  ...
  schedule(
    ...
    const criticalPath = windows.filter((window) => window.slackMs === 0).map((window) => window.taskId)
...
export class PlannerOptimizer {
   ...
   */
  optimize(tasks: readonly PlannerTask[], level: OptimizationLevel): PlannerTask[] {
    this.optimizations++
    ...
    switch (level) {
      ...
      case OptimizationLevel.Maximum:
        return this.prioritize(this.mergeParallelizable(this.dropRedundant(this.dropSkippable(tasks))))
  ...
  private dropSkippable(tasks: readonly PlannerTask[]): PlannerTask[] {
    return tasks.filter((task) => task.getState() !== TaskState.Skipped || task.getPriority() === TaskPriority.Critical)
  }
...
export class PlannerRollbackManager {
  ...
  private buildSteps(plan: Plan, policy: RollbackPolicy, checkpoints: readonly TaskId[]): string[] {
    ...
    const steps: string[] = [`Halt further task dispatch for plan '${plan.id}'.`]
  ...
  private buildSteps(plan: Plan, strategy: RecoveryStrategy): string[] {
    switch (strategy) {
      ...
      case RecoveryStrategy.Escalate:
        return ['Identify the failed task.', 'Surface the failure to a human or higher-level policy for a decision.']
      ...
      default:
        return [`Abort plan '${plan.id}'.`, 'Hand off to the rollback plan, if one exists.']
    }
  }
...
export class PlannerTransactionManager {
  ...
  describe(): string {
    return ['PlannerTransactionManager', `transactions=${this.totalCount()}`, `active=${this.activeCount()}`].join(', ')
  }
...
export class PlannerDiagnostics {
  ...
  runAll(plan: Plan, graph: PlannerDependencyGraph, snapshots: PlannerSnapshotManager): boolean {
    return (
...
export class PlannerManager implements PlannerFactory {
  ...
  private inferPlanType(graph: PlannerDependencyGraph): PlanType {
    ...
    const hasBranching = graph.nodes().some((id) => graph.dependents(id).size > 1 || graph.dependencies(id).size > 1)
  ...
  private optimizePlanInternal(plan: Plan, tasks: readonly PlannerTask[], optimizationLevel?: OptimizationLevel): Plan {
    const started = performance.now()
  ...
  private validatePlanInternal(plan: Plan): Plan {
    ...
    if (!validation.valid && this.configuration.strictValidation) {
      ...
      throw new Error(`Plan '${plan.id}' failed strict validation with ${validation.issues.length} issue(s).`)
    }
  ...
  addDependency(planId: PlanId, dependency: Dependency): Plan {
    ...
    if (!from || !to) {
      throw new Error(`Dependency references unknown task(s): '${dependency.fromTaskId}' -> '${dependency.toTaskId}'.`)
    }
    ...
    if (graph.hasCycle()) {
      throw new Error(`Dependency '${dependency.fromTaskId}' -> '${dependency.toTaskId}' would introduce a cycle.`)
    }

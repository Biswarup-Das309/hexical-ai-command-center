...
function sanitizeMetadata(
  ...
  if (serializedSize > maxBytes) {
    throw new RangeError(
      `metadata payload too large (${serializedSize} bytes, limit ${maxBytes}).`
    );
  }
...
function deepCopy<T>(value: T): T {
  const globalStructuredClone = (globalThis as { structuredClone?: <V>(v: V) => V })
    .structuredClone;
...
export class ReasoningSession {
  ...
  beginExecution(): void {
    if (this.executing) {
      throw new Error(
        `Session ${this.id} is already executing; concurrent executions on ` +

...
const RECOMMENDATION_PRIORITY_VALUES: ReadonlySet<number> = new Set(
  Object.values(RecommendationPriority).filter((value): value is number => typeof value === 'number'),
)
...
const RECOMMENDATION_CATEGORY_VALUES: ReadonlySet<string> = new Set(Object.values(RecommendationCategory))
...
export interface RecommendationEvidence {
  id: string
  source: 'severity' | 'semantic' | 'impact' | 'behavior' | 'rule' | 'heuristic' | 'future-ai'
  title: string
  description: string
  confidence: number
  weight: number
...
export interface ReviewerSuggestion {
  role: 'Backend' | 'Frontend' | 'Security' | 'DevOps' | 'QA' | 'Database' | 'Platform' | 'AI' | 'Architect' | 'General'
  reason: string
  priority: RecommendationPriority
}
...
export class RecommendationRuleRegistry {
  ...
  getRules(): RecommendationRule[] {
    return [...this.rules.values()].filter((rule) => rule.enabled).sort((a, b) => b.priority - a.priority)
  }
}
    ...
    for (const rule of this.registry.getRules()) {
      ...
      try {
        ...
        if (!Array.isArray(result)) {
          diagnostics.warnings.push(`Rule '${rule.name}' returned a non-array result and was ignored.`)
        } else {
          const capped = result.length > this.config.maxRecommendationsPerRule
          const usable = capped ? result.slice(0, this.config.maxRecommendationsPerRule) : result
          ...
          for (const candidate of usable) {
            if (!this.isValidRecommendation(candidate)) {
              diagnostics.warnings.push(`Rule '${rule.name}' produced a malformed recommendation; discarded.`)
...
export class EvidenceQuery {
  ...
  search(keyword: string): EvidenceNode[] {
    ...
    return this.graph
      .getNodes()
      .filter((node) => node.title.toLowerCase().includes(query) || node.description.toLowerCase().includes(query))
  }
}
...
export class ContradictionEngine {
  ...
  private isContradictory(a: EvidenceNode, b: EvidenceNode): boolean {
    return Math.abs(a.confidence - b.confidence) >= 0.4
  }

  private describe(a: EvidenceNode, b: EvidenceNode): string {
    return `Nodes '${a.title}' disagree in confidence (${(a.confidence * 100).toFixed(0)}% vs ${(
      b.confidence * 100
    ).toFixed(0)}%).`
  }

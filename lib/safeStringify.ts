...
function sanitizeOptions(options: SafeStringifyOptions): NormalizedOptions {
  return {
    ...
    maxCollectionSize: nonNegativeIntOr(options.maxCollectionSize, DEFAULT_OPTIONS.maxCollectionSize),
  ...
  try {
    ...
    if (value instanceof Map) {
      ...
      for (const [k, v] of value) {
        if (i >= options.maxCollectionSize) break
        out.push([normalize(k, seen, depth + 1, options, budget), normalize(v, seen, depth + 1, options, budget)])
        i++
        if (budget.used > options.maxOutputLength * BUDGET_MULTIPLIER) break
      }
      if (value.size > options.maxCollectionSize) {
    ...
    for (let i = 0; i < limit; i++) {
      const key = keys[i]
      result[key] = normalize((value as Record<string, unknown>)[key], seen, depth + 1, options, budget)
      if (budget.used > options.maxOutputLength * BUDGET_MULTIPLIER) break
    }

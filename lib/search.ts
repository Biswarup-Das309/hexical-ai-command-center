...
export function buildSearchIndex(nodes: ASTDiffNode[]): IndexedNode[] {
  ...
  return nodes.map((node) => {
    ...
    const searchable = [path, nodeType, operation].filter(Boolean).join(' ').toLowerCase()
...
export function searchIndex(index: IndexedNode[], query: string, options: SearchOptions = {}): SearchResult[] {
  const applyLimit = (results: SearchResult[]): SearchResult[] =>
    typeof options.maxResults === 'number' && options.maxResults >= 0 ? results.slice(0, options.maxResults) : results
 ...
 * or expands results.
 */
export function payloadContains(node: ASTDiffNode, term: string, stringify: (v: unknown) => string): boolean {

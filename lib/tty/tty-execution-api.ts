...
function safeOutputEvent(event: TTYOutputEvent): TTYBrowserOutputEvent | null {
  if (event.type !== 'stdout' && event.type !== 'stderr' && event.type !== 'state' && event.type !== 'completion') return null
...
export class TTYExecutionApi {
  ...
  async getExecution(executionId: TTYExecutionId, ownerUserId: string): Promise<TTYBrowserExecutionView | null> {
    ...
    const stdoutBytes = events.reduce((sum, event) => sum + (event.type === 'stdout' && typeof event.data.byteLength === 'number' ? event.data.byteLength : 0), 0)
    const stderrBytes = events.reduce((sum, event) => sum + (event.type === 'stderr' && typeof event.data.byteLength === 'number' ? event.data.byteLength : 0), 0)
    return {
  ...
  async getOutput(executionId: TTYExecutionId, ownerUserId: string, options?: { readonly start?: string; readonly end?: string; readonly count?: number }): Promise<readonly TTYBrowserOutputEvent[] | null> {
    const state = await this.authorizedState(executionId, ownerUserId)
    if (!state) return null
  ...
  private async authorizedState(executionId: TTYExecutionId, ownerUserId: string): Promise<TTYExecutionStateRecord | null> {

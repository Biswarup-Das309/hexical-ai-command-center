...
export const TTY_TIMELINE_STATES: readonly TTYExecutionState[] = [
  'queued',
  'leased',
  'starting',
  'running',
...
export function buildTTYExecutionTimeline(
  events: readonly TTYStreamEvent[],
  now = Date.now(),
): readonly TTYTimelineEntry[] {
  const stateEvents = events.filter(
    (event): event is Extract<TTYStreamEvent, { type: 'state' }> => event.type === 'state',
  )

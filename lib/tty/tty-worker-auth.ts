...
import { parseTTYWorkerId, isTTYWorkerCapability, type TTYWorkerAuthContext, type TTYWorkerCapability, type TTYWorkerId } from './tty-worker-types'
...
export function issueTTYWorkerToken(
  ...
  assertSecret(secret)
  if (parseTTYWorkerId(workerId) === null || !isTTYWorkerCapability(capability)) throw new Error('Invalid TTY worker token subject.')
  const nowMs = options.now?.() ?? Date.now()
  const ttlMs = options.ttlMs ?? DEFAULT_TOKEN_TTL_MS
  if (!Number.isSafeInteger(nowMs) || !Number.isSafeInteger(ttlMs) || ttlMs <= 0 || ttlMs > 365 * 24 * 60 * 60 * 1000) throw new Error('Invalid TTY worker token lifetime.')
...
export function verifyWorkerToken(
  ...
  try {
    ...
    if (supplied.length !== expected.length || !timingSafeEqual(supplied, expected)) return { valid: false, reason: 'invalid_token' }
    ...
    const capability = typeof record.capability === 'string' && isTTYWorkerCapability(record.capability) ? record.capability : null
...
function assertSecret(secret: string): void {
  if (typeof secret !== 'string' || secret.length < MIN_SECRET_LENGTH) throw new Error('TTY worker token secret is too short.')
}
...
export class TTYWorkerAuthenticator {
  ...
  async resolveWorkerContext(
    ...
    if (!worker.capabilities.includes(verification.capability) || (requiredCapability !== undefined && verification.capability !== requiredCapability && verification.capability !== 'execute')) {
  ...
  async rejectUnknownWorker(workerId: TTYWorkerId): Promise<TTYWorkerAuthResult | null> {
    return (await this.registry.getWorker(workerId)) === null ? { authenticated: false, reason: 'unknown_worker' } : null
  }

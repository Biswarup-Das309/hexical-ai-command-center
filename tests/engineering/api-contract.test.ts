...
test('engineering routes authenticate, use bounded snapshots, and keep unsupported checks explicit', async () => {
  ...
  const detail = await readFile('app/api/engineering/runs/[runId]/route.ts', 'utf8')
  ...
  const panel = await readFile('components/engineering/EngineeringTaskPanel.tsx', 'utf8')

  assert.match(ingest, /await auth\(\)/)
  assert.match(ingest, /export async function GET\(\)/)
  assert.match(ingest, /listRuns\(ownerUserId\)/)
  assert.match(ingest, /REPOSITORY_MAX_TOTAL_BYTES/)
  assert.match(ingest, /createRepository\(ownerUserId, graph\)/)
  assert.match(detail, /await auth\(\)/)
  assert.match(detail, /getRun\(ownerUserId, runId\)/)
  ...
  assert.match(store, /eq\('owner_user_id', ownerUserId\)/)
  assert.match(store, /async listRuns\(ownerUserId: string/)
  assert.match(store, /input_context: asJson\(input.inputContext/)
  assert.match(store, /evidence: asJson\(input.evidence/)
  assert.match(panel, /ENGINEERING_ACTIVE_RUN_STORAGE_KEY/)
  assert.match(panel, /restoreEngineeringRun/)
  assert.match(panel, /Loading previous engineering run/)
  assert.match(panel, /No previous engineering run/)
})

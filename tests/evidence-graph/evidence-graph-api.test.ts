...
async function fixture() {
  ...
  const graphStore = new EvidenceGraphStore(new FakeEvidenceGraphRedis(), {
    getInvestigation: async (ownerUserId, investigationId) => {
      ...
      return hydration
        ? { investigationId, title: hydration.investigation.title, status: hydration.investigation.status }
        : null
    },
  })
  const observation: EvidenceGraphObservation = {
    ...
    extraction: {
      entities: [{ type: 'host', canonicalKey: 'api.example.com', label: 'api.example.com', value: 'api.example.com' }],
      relationships: [],
    },
  }
...
test('graph APIs enforce authentication, owner isolation, browser-safe responses, and pagination validation', async () => {
  ...
  const summary = await fixtureData.api.summary(
    request(`/api/investigations/${fixtureData.investigation.investigationId}/graph/summary`),
    fixtureData.investigation.investigationId,
  )
  ...
  const entities = await fixtureData.api.entities(
    request(`/api/investigations/${fixtureData.investigation.investigationId}/graph/entities?type=host&limit=1`),
    fixtureData.investigation.investigationId,
  )
  ...
  const invalid = await fixtureData.api.entities(
    request(`/api/investigations/${fixtureData.investigation.investigationId}/graph/entities?type=unknown`),
    fixtureData.investigation.investigationId,
  )
  assert.equal(invalid.status, 400)

  fixtureData.setUser(OTHER)
  const denied = await fixtureData.api.graph(
    request(`/api/investigations/${fixtureData.investigation.investigationId}/graph`),
    fixtureData.investigation.investigationId,
  )
...
test('graph entity details and connected evidence remain owner-scoped', async () => {
  const fixtureData = await fixture()
  const entitiesResponse = await fixtureData.api.entities(
    request(`/api/investigations/${fixtureData.investigation.investigationId}/graph/entities?type=host`),
    fixtureData.investigation.investigationId,
  )
  ...
  const entity = await fixtureData.api.entity(
    request(`/api/investigations/${fixtureData.investigation.investigationId}/graph/entities/${entityId}`),
    fixtureData.investigation.investigationId,
    entityId,
  )
  assert.equal(entity.status, 200)
  const connected = await fixtureData.api.connected(
    request(`/api/investigations/${fixtureData.investigation.investigationId}/graph/entities/${entityId}/connected`),
    fixtureData.investigation.investigationId,
    entityId,
  )
  assert.equal(connected.status, 200)
  const execution = await fixtureData.api.execution(
    request(`/api/investigations/${fixtureData.investigation.investigationId}/graph/executions/${EXECUTION_ID}`),
    fixtureData.investigation.investigationId,
    EXECUTION_ID,
  )
...
test('summary recreates the investigation graph root before querying an empty graph', async () => {
  ...
  const response = await api.summary(
    request(`/api/investigations/${investigation.investigationId}/graph/summary`),

...
function collect(lines: readonly string[]) {
  ...
  const results = lines.map((text, index) => engine.extract(EXECUTION_ID, { type: 'stdout' as const, text, sequence: index + 1, timestamp: `2026-08-09T12:00:0${index}.000Z` }))
  return [...results, engine.flush(EXECUTION_ID)]
}

test('extracts Nmap hosts, IPs, ports, services, and versions deterministically', () => {
  const results = collect(['Nmap scan report for app.example.com (192.0.2.10)\n', '80/tcp open http Apache httpd 2.4.58\n'])
...
test('handles malformed and partial output without throwing or producing unstable duplicates', () => {
  const engine = new DeterministicExtractionEngine()
  engine.reset(EXECUTION_ID)
  assert.doesNotThrow(() => engine.extract(EXECUTION_ID, { type: 'stderr', text: '\u0000\u0001 not structured output', sequence: 1, timestamp: '2026-08-09T12:00:00.000Z' }))
  const first = engine.extract(EXECUTION_ID, { type: 'stdout', text: 'Nmap scan report for partial.example.com', sequence: 2, timestamp: '2026-08-09T12:00:01.000Z' })
  assert.equal(first.entities.length, 0)
  const second = engine.extract(EXECUTION_ID, { type: 'stdout', text: ' (192.0.2.11)\n', sequence: 3, timestamp: '2026-08-09T12:00:02.000Z' })
  assert.ok(second.entities.some(entity => entity.type === 'host'))
  const replay = engine.extract(EXECUTION_ID, { type: 'stdout', text: 'CVE-2024-12345\n', sequence: 4, timestamp: '2026-08-09T12:00:03.000Z' })

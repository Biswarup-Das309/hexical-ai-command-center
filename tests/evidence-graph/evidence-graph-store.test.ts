...
test('persists immutable graph entities, directional edges, and prevents duplicate writes', async () => {
  ...
  const [first, second] = await Promise.all([
    graphStore.upsertObservations(OWNER, investigation.investigationId, [item]),
    graphStore.upsertObservations(OWNER, investigation.investigationId, [item]),
  ])
  ...
  assert.ok(connected)
  assert.ok(
    connected.relationships.some(
      (edge) =>
        edge.source === graphEntityId(investigation.investigationId, 'investigation', investigation.investigationId) &&
        edge.target === host.id,
    ),
  )
...
test('supports bounded entity and relationship pagination and restart recovery', async () => {
  ...
  const observations = Array.from({ length: 135 }, (_, index) =>
    observation(investigation.investigationId, EXECUTION_ID, index + 1, `host-${index}.example.com`),
  )
  ...
  const second = await graphStore.listEntities(OWNER, investigation.investigationId, {
    type: 'host',
    limit: 20,
  ...
  const restarted = new EvidenceGraphStore(graphRedis, {
  ...
  const large: EvidenceGraphObservation = {
    ...
    extraction: {
      entities: Array.from({ length: 10_000 }, (_, index) => ({
        ...
        canonicalKey: `host-${index}.example.com`,
        label: `host-${index}.example.com`,
        value: `host-${index}.example.com`,
        metadata: { parser: 'large-test' },
      })),
      relationships: [],
    },
  }

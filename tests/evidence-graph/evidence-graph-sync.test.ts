...
function execution(): TTYBrowserExecutionView {
  return {
    ...
    state: 'succeeded',
    timestamps: { queuedAt: CREATED_AT, updatedAt: '2026-08-09T12:10:05.000Z', leasedAt: CREATED_AT, startedAt: '2026-08-09T12:10:01.000Z', finishedAt: '2026-08-09T12:10:05.000Z' },
    outputSummary: { eventCount: 5, stdoutBytes: 100, stderrBytes: 0, totalBytes: 100, lastEventAt: '2026-08-09T12:10:05.000Z' },
...
function source(output: readonly TTYBrowserOutputEvent[], investigationStore: InvestigationStore) {
  return {
    getInvestigation: async (ownerUserId: string, id: InvestigationId) => {
      ...
      return hydrated ? { investigationId: hydrated.investigation.investigationId, title: hydrated.investigation.title, status: hydrated.investigation.status } : null
    },
    getExecutions: async (ownerUserId: string, id: InvestigationId) => (await investigationStore.get(ownerUserId, id, { executionLimit: 50, timelineLimit: 1 }))?.executions ?? [],
...
async function fixture(output: readonly TTYBrowserOutputEvent[]) {
  ...
  await investigationStore.attachExecution(OWNER, investigation.investigationId, { executionId: EXECUTION_ID, sessionId: SESSION_ID, attachedAt: CREATED_AT })
  const graphRedis = new FakeEvidenceGraphRedis()
  const graphStore = new EvidenceGraphStore(graphRedis, { getInvestigation: async (ownerUserId, id) => source(output, investigationStore).getInvestigation(ownerUserId, id) })
...
test('synchronizes streaming output into a graph and remains replay-safe', async () => {
  const output: TTYBrowserOutputEvent[] = [
    { sequence: 1, timestamp: '2026-08-09T12:10:01.000Z', type: 'stdout', text: 'Nmap scan report for app.example.com (192.0.2.20)\n', state: null },
    { sequence: 2, timestamp: '2026-08-09T12:10:02.000Z', type: 'stdout', text: '443/tcp open https nginx 1.25\n', state: null },
    { sequence: 3, timestamp: '2026-08-09T12:10:03.000Z', type: 'stdout', text: 'https://app.example.com\nHTTP/1.1 200 OK\n', state: null },
...
test('incremental synchronization survives replacement synchronizers and appends only new output', async () => {
  const output: TTYBrowserOutputEvent[] = [{ sequence: 1, timestamp: '2026-08-09T12:10:01.000Z', type: 'stdout', text: 'Nmap scan report for incremental.example.com (192.0.2.21)\n', state: null }]
  ...
  output.push({ sequence: 2, timestamp: '2026-08-09T12:10:02.000Z', type: 'stdout', text: '8080/tcp open http Caddy 2.7\n', state: null })
  const replacement = new EvidenceGraphSynchronizer(fixtureData.graphStore, source(output, fixtureData.investigationStore))
...
test('concurrent graph synchronization is idempotent and malformed output is ignored safely', async () => {
  const output: TTYBrowserOutputEvent[] = [
    { sequence: 1, timestamp: '2026-08-09T12:10:01.000Z', type: 'stdout', text: '\u0000\u0001 malformed\n', state: null },

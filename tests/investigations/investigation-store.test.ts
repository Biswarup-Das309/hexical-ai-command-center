...
} from '../../lib/investigations/investigation-keys'
...
test('persists investigation metadata, timeline, bookmarks, notes, and execution attachment idempotently', async () => {
  ...
  const created = await investigations.create(
    OWNER,
    { title: 'Credential exposure review', description: 'Review approved evidence.' },
    FIXED_TIME,
  )

  assert.equal(created.ownerUserId, OWNER)
  assert.equal(created.status, 'active')
  const attached = await investigations.attachExecution(OWNER, created.investigationId, {
    executionId: '00000000-0000-4000-8000-000000000902',
    sessionId: SESSION_ID,
    attachedAt: FIXED_TIME,
  })
  const duplicate = await investigations.attachExecution(OWNER, created.investigationId, {
    executionId: '00000000-0000-4000-8000-000000000902',
    sessionId: SESSION_ID,
    attachedAt: FIXED_TIME,
  })
  ...
  await investigations.recordBookmark(
    OWNER,
...
test('paginates investigations and execution history without loading the full index', async () => {
  ...
  const records = await Promise.all(
    Array.from({ length: 5 }, (_, index) =>
      investigations.create(OWNER, { title: `Case ${index}`, description: '' }, `2026-08-09T10:00:0${index}.000Z`),
    ),
  )
  ...
  assert.equal(
    new Set(
      [...first.investigations, ...second.investigations, ...third.investigations].map((item) => item.investigationId),
    ).size,
    records.length,
  )
})
...
test('paginates past stale owner-index entries without underfilling a page', async () => {
  const redis = new FakeInvestigationRedis()
  const investigations = store(redis)
  const first = await investigations.create(
    OWNER,
    { title: 'First visible', description: '' },
    '2026-08-09T10:00:00.000Z',
  )
  const second = await investigations.create(
    OWNER,
    { title: 'Second visible', description: '' },
    '2026-08-09T10:01:00.000Z',
  )
  const third = await investigations.create(
    OWNER,
    { title: 'Third visible', description: '' },
  ...
  assert.deepEqual(
    page.investigations.map((item) => item.investigationId),
    [second.investigationId, first.investigationId],
  )
  assert.equal(page.investigations.length, 2)
  assert.equal(page.nextCursor, null)
  assert.equal(
    (
      await redis.zrange<string[]>(investigationOwnerIndexKey(OWNER), 0, -1, { rev: true, offset: 0, count: 10 })
    ).includes(third.investigationId),
    false,
  )
})
...
test('attaches one durable TTY session per investigation and removes it during deletion', async () => {
  ...
  assert.equal(
    (await investigations.get(OWNER, created.investigationId))?.investigation.ttySessionId,
    attached[0]?.ttySessionId,
  )
  assert.equal(
    (await investigations.get(OWNER, created.investigationId))?.timeline.some(
      (event) => event.type === 'session_attached',
    ),
    true,
  )
...
test('edits and deletes notes through replayable timeline events', async () => {
  ...
  await investigations.updateNote(
    OWNER,
    created.investigationId,
    original.noteId,
    'Edited note',
...
test('archives, restores, soft-deletes, and hides an investigation while retaining an audit timeline', async () => {
  ...
  const archived = await investigations.patch(
    OWNER,
    created.investigationId,
    { status: 'archived' },
    '2026-08-09T10:01:00.000Z',
  ...
  const restored = await investigations.patch(
    OWNER,
    created.investigationId,
    { status: 'active' },
    '2026-08-09T10:02:00.000Z',
  ...
  assert.equal(
    (await investigations.list(OWNER)).investigations.some((item) => item.investigationId === created.investigationId),
    false,
  )
...
test('a fresh store instance rehydrates persisted execution state after a worker restart', async () => {
  ...
  await firstStore.attachExecution(OWNER, created.investigationId, {
    executionId,
    sessionId: SESSION_ID,
    attachedAt: FIXED_TIME,
  })
  await firstStore.updateExecution(OWNER, created.investigationId, executionId, 'running', {
...
test('execution history refuses to resurrect a terminal execution', async () => {
  ...
  await investigations.attachExecution(OWNER, created.investigationId, {
    executionId,
    sessionId: SESSION_ID,
    attachedAt: FIXED_TIME,
  })
  await investigations.updateExecution(OWNER, created.investigationId, executionId, 'succeeded', {
    updatedAt: '2026-08-09T10:00:02.000Z',
  })
  const replayed = await investigations.updateExecution(OWNER, created.investigationId, executionId, 'queued', {
...
test('timeline synchronization is idempotent across reconnects', async () => {
  ...
  await investigations.recordExecutionEvent(OWNER, created.investigationId, {
    type: 'stdout',
    executionId,
    sequence: 1,

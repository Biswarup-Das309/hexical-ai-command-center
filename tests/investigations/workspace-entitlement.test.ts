...
test('resolves an active Pro profile for the same owner identity used by TTY lifecycle', () => {
  const entitlement = resolveWorkspaceEntitlement({ tier: 'pro', subscription_status: 'active', current_period_end: FUTURE }, NOW)
...
test('resolves Free and missing profiles as non-entitled', () => {
  assert.equal(resolveWorkspaceEntitlement({ tier: 'free', subscription_status: 'active', current_period_end: FUTURE }, NOW).tier, 'free')
...
test('uses fresh profile data after entitlement hydration so an upgraded user becomes Pro', () => {
  const free = resolveWorkspaceEntitlement({ tier: 'free', subscription_status: 'active', current_period_end: FUTURE }, NOW)
  const upgraded = resolveWorkspaceEntitlement({ tier: 'pro', subscription_status: 'active', current_period_end: FUTURE }, NOW)

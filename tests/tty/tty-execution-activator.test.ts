...
test('activation resolves only after the coordinator has persisted acceptance and deduplicates concurrent calls', async () => {
  ...
  const activator = new TTYExecutionActivator({
    coordinator: {
      getState: async () => null,
      run: async (
        _executionId: typeof executionId,
        _sessionId: typeof sessionId,
        receivedOptions: TTYExecutionCoordinatorRunOptions = {},
      ) => {
        calls += 1
...
test('activation surfaces coordinator rejection and logs its reason', async () => {
  ...
  const activator = new TTYExecutionActivator({
    ...
    onFailure: (failure) => {
      failures.push(`${failure.phase}:${failure.reason}`)
    },
  })

  assert.deepEqual(await activator.activate(executionId, sessionId), {
    accepted: false,
    state: null,
    reason: 'missing_job',
  })

...
test('rejects unauthenticated and malformed admission requests', async () => {
  const unauthenticated = await api({ authenticate: async () => null }).admit(request({ input: 'help', idempotencyKey: 'abcdefghijklmnop' }), session.sessionId)
  assert.equal(unauthenticated.status, 401)
  const malformed = await api().admit(request({ input: 'help', idempotencyKey: 'short', kindHint: 'recon_probe' }), session.sessionId)
  assert.equal(malformed.status, 400)
})

test('derives classification and returns only the browser-safe queued job', async () => {
  let seenKind: string | undefined
  const result = await api({ admission: { admit: async (_args: unknown) => { seenKind = 'recon_probe'; return { admitted: true, job, duplicate: false } } } as never }).admit(request({ input: 'recon 10.0.0.1', idempotencyKey: 'abcdefghijklmnop' }), session.sessionId)
...
test('fails closed when authorization denies or the session is not owned', async () => {
  ...
  const missing = await api({ getSession: async () => null }).admit(request({ input: 'help', idempotencyKey: 'abcdefghijklmnop' }), session.sessionId)
  assert.equal(missing.status, 404)
})

test('does not admit non-entitled or terminal sessions', async () => {
  const tierDenied = await api({ resolveTier: async () => 'free' }).admit(request({ input: 'help', idempotencyKey: 'abcdefghijklmnop' }), session.sessionId)
  assert.equal(tierDenied.status, 403)
  const terminated = await api({ getSession: async () => ({ ...session, status: 'terminated' }) }).admit(request({ input: 'help', idempotencyKey: 'abcdefghijklmnop' }), session.sessionId)

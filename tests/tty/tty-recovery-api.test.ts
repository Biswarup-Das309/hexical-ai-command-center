...
test('recovery scans the active index, cleans an orphan, and delegates state mutation', async () => {
  ...
  const running = transitionTTYExecutionState(
    transitionTTYExecutionState(queued, 'leased', '2026-08-08T10:00:00.010Z', {
      workerId: 'worker-old' as never,
      leaseId: 'lease-old' as never,
    }),
    'starting',
    '2026-08-08T10:00:00.020Z',
  )
  ...
  const recovery = new TTYRecoveryManager(redis as never, {
  ...
  const result = await recovery.reconcile(async (id) => {
    ...
    return recoverTTYExecutionState(running, '2026-08-08T10:00:00.030Z', {
      workerId: null,
      leaseId: null,
...
test('browser-safe execution API verifies session ownership and omits worker and lease internals', async () => {
  ...
  const running = transitionTTYExecutionState(
    transitionTTYExecutionState(queued, 'leased', '2026-08-08T10:00:00.010Z', {
      workerId: 'worker-api' as never,
      leaseId: 'opaque-api-lease' as never,
    }),
    'starting',
    '2026-08-08T10:00:00.020Z',
  )

...
function session(
  sessionId: ReturnType<typeof createTTYSessionId>,
  ownerUserId: string,
  status: InternalTTYSession['status'] = 'active',
): InternalTTYSession {
  return {
    sessionId,
    ownerUserId,
    ...
    usage: {
      activeSessions: 1,
      activeExecutionsInSession: 1,
      executionsInLastMinute: 1,
      queueDepth: 0,
...
test('authorizer permits only the owner of the execution session', async () => {
  ...
  assert.deepEqual(await authorizer.authorize({ userId: null, executionId }), {
    authorized: false,
    reason: 'unauthenticated',
  })
  assert.deepEqual(await authorizer.authorize({ userId: 'other', executionId }), {
    authorized: false,
    reason: 'session_not_found',
  })
  assert.deepEqual(
    await authorizer.authorize({ userId: 'owner', executionId, requestedSessionId: createTTYSessionId() }),
    { authorized: false, reason: 'session_not_found' },
  )
  assert.deepEqual(await authorizer.authorize({ userId: 'owner', executionId }), {
    authorized: true,
    userId: 'owner',
    executionId,
    sessionId,
...
test('authorizer fails closed for missing, inactive, and denied executions', async () => {
  ...
  assert.deepEqual(await missing.authorize({ userId: 'owner', executionId }), {
    authorized: false,
    reason: 'execution_not_found',
  })

  const inactive = new TTYStreamAuthorizer({
    getExecutionState: async () => state,
    getSession: async (id) => session(id, 'owner', 'terminated'),
  })
  assert.deepEqual(await inactive.authorize({ userId: 'owner', executionId }), {
    authorized: false,
    reason: 'session_not_active',
  })

  const denied = new TTYStreamAuthorizer({
    getExecutionState: async () => state,
    getSession: async (id) => session(id, 'owner'),
    canSubscribe: async () => false,
  })
  assert.deepEqual(await denied.authorize({ userId: 'owner', executionId }), {
    authorized: false,

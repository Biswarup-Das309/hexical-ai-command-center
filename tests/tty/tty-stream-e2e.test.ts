...
test('one execution remains ordered and leak-free for 100 concurrent viewers', async () => {
  ...
  const subscriptions = await Promise.all(received.map((events, index) => broker.subscribe(executionId, event => events.push(event.sequence), index)))

  for (let index = 0; index < 24; index += 1) {
    if (index % 3 === 0) await broker.publish({ executionId, sessionId, type: 'state', payload: { state: 'streaming' } })
    else await broker.publish({ executionId, sessionId, type: 'stdout', payload: { text: `chunk-${index}`, byteLength: 8 } })
  }
  await broker.publish({ executionId, sessionId, type: 'completion', payload: { state: 'cancelled', exitCode: null, signal: 'SIGTERM', failureCode: 'CANCELLED' } })

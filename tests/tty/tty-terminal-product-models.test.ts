...
import { createTTYEvidenceBookmark, parseTTYEvidenceBookmarks, serializeTTYEvidenceBookmarks } from '@/lib/tty/tty-evidence-bookmarks'
...
function state(sequence: number, value: 'queued' | 'running' | 'streaming') {
  return createTTYStreamEvent({ executionId, sessionId, sequence, timestamp: `2026-08-09T00:00:0${sequence}.000Z`, type: 'state', payload: { state: value } })
}
function output(sequence: number, text: string) {
  return createTTYStreamEvent({ executionId, sessionId, sequence, timestamp: '2026-08-09T00:00:10.000Z', type: 'stdout', payload: { text, byteLength: text.length } })
}

test('timeline and search models preserve state order and partial output lines', () => {
  const timeline = buildTTYExecutionTimeline([state(1, 'queued'), state(2, 'running'), state(3, 'streaming')], Date.parse('2026-08-09T00:00:04.000Z'))
  assert.deepEqual(timeline.map(item => item.state), ['queued', 'running', 'streaming'])
  assert.equal(timelineDurationLabel(1_500), '1.50s')
  const lines = buildTTYTerminalLines([output(4, 'first\nsecond'), output(5, ' line\nthird')])
  assert.deepEqual(lines.map(line => line.text), ['first', 'second line', 'third'])
...
test('evidence bookmarks round-trip with an execution-scoped local format', () => {
  const bookmark = createTTYEvidenceBookmark({ executionId, sequence: 4, lineNumber: 2, kind: 'output', label: 'interesting path', excerpt: 'second line' })

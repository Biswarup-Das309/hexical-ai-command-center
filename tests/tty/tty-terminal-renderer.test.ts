...
function output(sequence: number, text: string, type: 'stdout' | 'stderr' = 'stdout') {
  return createTTYStreamEvent({ executionId, sessionId, sequence, timestamp: '2026-08-09T01:02:03.000Z', type, payload: { text, byteLength: Buffer.byteLength(text) } })
}
...
test('renderer converts lifecycle events to timestamped system lines and ignores duplicates', () => {
  const writes: string[] = []
  const state = createTTYStreamEvent({ executionId, sessionId, sequence: 3, timestamp: '2026-08-09T01:02:03.000Z', type: 'state', payload: { state: 'running' } })
  const duplicate = createTTYStreamEvent({ executionId, sessionId, sequence: 3, timestamp: '2026-08-09T01:02:03.000Z', type: 'state', payload: { state: 'running' } })

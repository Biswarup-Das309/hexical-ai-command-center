...
import { TTYExecutionCoordinator, type TTYExecutionCoordinatorRunHooks, type TTYExecutionRunResult } from '../../lib/tty/tty-execution-coordinator'
...
import { TTYWorkerClaimService, type TTYWorkerClaimLogger, type TTYWorkerCoordinatorClaimResult, type TTYWorkerOwnership } from '../../lib/tty/tty-worker-claim'
import { TTYWorkerExecutor, type TTYWorkerExecutorDependencies, type TTYWorkerExecutorLogger } from '../../lib/tty/tty-worker-executor'
import { TTYWorkerPoller } from '../../lib/tty/tty-worker-poller'
import type { TTYLeasedJob, TTYLeaseClaimResult, TTYLeaseCompleteResult, TTYLeaseReleaseResult, TTYLeaseRenewResult } from '../../lib/tty/tty-execution-lease'
...
const session: InternalTTYSession = {
  ...
  limits: { maxConcurrentSessions: 1, maxConcurrentExecutionsPerSession: 1, maxExecutionsPerMinute: 20, maxExecutionDurationMs: 5_000, maxSessionIdleMs: 900_000, maxSessionDurationMs: 3_600_000, maxOutputBytesPerExecution: 64_000, maxQueueDepth: 10 },
  usage: { activeSessions: 1, activeExecutionsInSession: 0, executionsInLastMinute: 0, queueDepth: 0, capturedAt: '2026-08-09T10:00:00.000Z' }
}
...
class PollerStub {
  ...
  async startPolling(): Promise<void> { this.startCalls += 1 }
  async stopPolling(): Promise<void> { this.stopCalls += 1 }
}
...
function terminalState(id: TTYExecutionId, state: 'succeeded' | 'failed' | 'cancelled' | 'timed_out' | 'expired'): TTYExecutionStateRecord {
  const queued = createQueuedTTYExecutionState(id, sessionId, '2026-08-09T10:00:00.000Z')
  const leased = transitionTTYExecutionState(queued, 'leased', '2026-08-09T10:00:00.001Z', { workerId, leaseId: createTTYLeaseId('executor-safe-lease') })
...
function createFakeClaim(options: {
  ...
  return {
    ...
    get forgetCalls() { return forgetCalls },
    ...
    forgetOwnership: (_current: TTYWorkerOwnership) => { forgetCalls += 1 }
  }
}

function createFakeCoordinator(options: {
...
test('runtime/coordinator failure releases ownership and invokes recovery without leaking secrets', async () => {
  ...
  const harness = createExecutor({
    ...
    recovery: { recoverNow: async () => { recoveryCalls += 1 } }
  })
  await harness.executor.start()
  const result = await harness.executor.executeExecution(executionId)
...
test('timeout, cancellation, and lease loss remain explicit and idempotent', async () => {
  const timeout = createExecutor({ coordinator: createFakeCoordinator({ result: { accepted: true, state: terminalState(executionId, 'timed_out') } }) })
  ...
  const cancelled = createExecutor({ coordinator: createFakeCoordinator({ result: { accepted: true, state: terminalState(executionId, 'cancelled') } }) })
...
test('restart during an active execution runs recovery before the replacement polls', async () => {
  ...
  const first = createExecutor({
    coordinator: createFakeCoordinator({ gate }),
    recovery: { recoverNow: async () => { initialRecoveryCalls += 1 } }
  })
  ...
  const restarted = createExecutor({ recovery: { recoverNow: async () => { restartRecoveryCalls += 1 } } })
...
test('poll, claim, execute, stream, complete, and recovery compose through the real coordinator and stream bridge', async () => {
  ...
  const leases = {
    ...
    release: async (): Promise<TTYLeaseReleaseResult> => ({ released: true, job: { ...job, status: 'queued' } as never })
  }
  ...
  const coordinator = new TTYExecutionCoordinator({
    ...
    resourceGuard: new TTYResourceGuard({ maxConcurrentProcesses: 1, maxStdoutBytesPerSecond: 64_000, maxStderrBytesPerSecond: 64_000 }),
    outputStream,
    commandAllowlist: { diagnostic: [basename(process.execPath).toLowerCase().replace(/\.exe$/, '')] },
  ...
  try {
    ...
    assert.equal(output.some(event => event.type === 'stdout' && event.data.text === 'worker-out'), true)
    assert.equal(output.some(event => event.type === 'stderr' && event.data.text === 'worker-err'), true)
    assert.equal(output.some(event => event.type === 'completion'), true)
    const live = await broker.replay(executionId)
    assert.equal(live.events.some(event => event.type === 'stdout'), true)
    assert.equal(live.events.some(event => event.type === 'stderr'), true)
    await executor.stop()
  } finally {
    broker.close()
